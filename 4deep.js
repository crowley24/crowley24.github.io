...
var epgTemplate = $(('<div id="PLUGIN_epg">\n' +  
	...
	'   <div class="PLUGIN-details__program-title">Потім</div>\n' +  
...
function channelSwitch(dig, isChNum) {  
	...
	if (number && number <= cnt) {  
		...
		if (isChNum === true) {  
			...
			// Ще одна цифра неможлива - перемикаємо  
			chSwitch();  
		} else {  
			// Чекаємо наступну цифру або перемикаємо  
	...
	this.create = function () {  
		...
		if (Object.keys(catalog).length) {  
			...
			var parseListHeader = function () {  
				...
				if (/^https?:\/\/.+/i.test(listCfg['epgUrl']) && listCfg['epgUrl'].length < 8000) {  
					channelsUri = listCfg['epgCode'] + '/' + channelsUri + '?url=' + encodeURIComponent(listCfg['epgUrl'])  
						+ '&sig=' + generateSigForString(listCfg['epgUrl']);  
				}  
			...
			var parseList = function () {  
				...
				var l = data.split(/\r?\n/);  
				var cnt = 0, i = 1, chNum = 0, m, mm, defGroup = defaultGroup, chInGroupCnt = {};  
				while (i < l.length) {  
					chNum = cnt + 1;  
					var channel = {  
						ChNum: chNum,  
			...
			network.native(  
				...
				function () {  
					network.silent(  
						...
						+ '&sig=' + generateSigForString(listUrl),  
						compileList,  
						emptyResult,  
						false,  
						{dataType: 'text'}  
...
if (!Lampa.Lang) {  
	...
	Lampa.Lang = {  
		...
		translate: function translate(key) {  
			return lang_data[key] ? lang_data[key].uk : key;  
		}  
	};  
}  
var langData = {};  
...
function pluginStart() {  
	...
	isSNG = Lampa.Storage.field('language') === 'uk';  
