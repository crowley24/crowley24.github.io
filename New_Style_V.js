(function () {  
    ...
    function applyGlobalCardFix() {  
        ...
        // Видаляємо примусове встановлення card_wide = true  
        // оскільки ми хочемо вертикальні картки  
    }  
    ...
    function decorateCard(state, card) {  
        ...
        if (!card.params.style.name) card.params.style.name = 'default';  
...
.new-interface .card.card--wide + .card-more .card-more__box {  
    ...
    const logoImg = $('<img>')  
        ...
        .css({  
            'max-height': '125px',  
            'margin-top': '-10px', // Змінено для підняття логотипу  
            'display': 'block'  
        })  
        .on('error', function() {  
...
function displayLogoInFullView(e, logoPath){  
    if(logoPath != ''){  
        e.object.activity.render().find('.full-start-new__title').html('<img style="margin-top:-10px;max-height:125px;" src="' + Lampa.TMDB.image('/t/p/w300' + logoPath.replace('.svg','.png')) + '"/>');  
    }  
}
