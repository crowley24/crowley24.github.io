(function () {          
    ...
    const TRANSLATIONS = {          
        ...
        'settings_cas_slideshow_interval': 'Інтервал слайдшоу',          
...
async function loadMovieDataOptimized(render, data) {          
    ...
    if (Lampa.Storage.get('cas_show_rating')) {          
        tasks.push(Promise.resolve().then(() => {          
            ...
            render.find('.cas-rate-items').html(ratesHtml).css('opacity','1').show();          
        }));          
    }          
    if (Lampa.Storage.get('cas_show_quality')) {          
        tasks.push(Promise.resolve().then(() => {          
            const quality = (data.release_date || data.first_air_date) ?   
                (new Date(data.release_date || data.first_air_date).getFullYear() >= 2020 ? '4K' : 'FULL HD') : 'HD';          
            const qualityHtml = QUALITY_ICONS[quality] ? `<div class="cas-quality-item"><img src="${QUALITY_ICONS[quality]}"></div>` : '';  
            render.find('.cas-quality-row').html(qualityHtml).css('opacity','0.9').show();          
        }));          
    }          
    if (Lampa.Storage.get('cas_show_studios')) {          
        tasks.push(Promise.resolve().then(() => {          
            if (data.production_companies && data.production_companies.length > 0) {          
                const studiosHtml = data.production_companies.slice(0, 5).map(company => {          
                    const logoPath = company.logo_path ? Lampa.TMDB.image('/t/p/h30' + company.logo_path) : null;          
                    return logoPath ? `<div class="cas-studio-item"><img src="${logoPath}" alt="${company.name}"></div>` : '';          
                }).join('');          
                render.find('.cas-studios-row').html(studiosHtml).css('opacity','0.9').show();          
            }          
        }));          
    }          
    Promise.all(tasks);          
...
const debouncedLoadMovieData = debounce(loadMovieDataOptimized, 250);          
