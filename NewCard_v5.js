(function () {
    ...
    var settings_list = [
        ...
        { id: 'mobile_interface_slideshow_quality', default: 'w780' } // Нове налаштування якості
    ];
    ...
    // Допоміжна функція для отримання найкращої якості з результатів парсера
    ...
    function startSlideshow($poster, backdrops) {
        ...
        var quality = Lampa.Storage.get('mobile_interface_slideshow_quality', 'w780'); // Отримуємо вибрану якість
    ...
    function initPlugin() {
        Lampa.Listener.follow('full', function (e) {
            ...
            if (window.innerWidth <= 480 && (e.type === 'complite' || e.type === 'complete')) {
                ...
                // Завантаження логотипу та бекдропів для слайд-шоу
                $.ajax({
                ...
                if ($details.length) {
                    ...
                    // Отримання та відображення якості (через парсер)
    ...
    function addSettings() {
        ...
        // --- НОВЕ: Вибір якості картинок ---
        Lampa.SettingsApi.addParam({
            component: 'mobile_interface',
            param: { 
                name: 'mobile_interface_slideshow_quality', 
                type: 'select', 
                values: { 'w300': 'Низька (w300)', 'w780': 'Середня (w780)', 'w1280': 'Висока (w1280)', 'original': 'Оригінал' }, 
                default: 'w780' 
            },
            field: { name: 'Якість слайд-шоу', description: 'Розмір зображень для фонового слайд-шоу' }
        });
        ...
        Lampa.SettingsApi.addParam({
            ...
            field: { name: 'Значки якості', description: 'Показувати 4K, HDR, Audio, UKR (через парсер)' }
        });
    }
    ...
    function start() {
        ...
        addSettings();
        initPlugin();
        // Примусове вимкнення блюру для мобілок, щоб картинка була чіткою
