(function () {  
    ...
    const defaults = {  
        ...
        'applecation_slideshow_time': '10000',  
        'applecation_slideshow_quality': 'w780',  
        'applecation_quality': true  
    };  
    Object.keys(defaults).forEach(key => {  
        if (Lampa.Storage.get(key, 'unset') === 'unset') Lampa.Storage.set(key, defaults[key]);  
    });  
    ...
    function getMediaType(data) {  
        ...
        const types = { uk: isTv ? 'Серіал' : 'Фільм' };  
        return types['uk'];  
    }  
  
    // Завантаження іконки студії з інвертуванням темних  
    function loadNetworkIcon(render, data) {  
        ...
        if (!showStudio) { networkContainer.remove(); return; }  
        const logos = [];  
        const collect = arr => {  
        ...
        collect(data.production_companies);  
        if (!logos.length) { networkContainer.remove(); return; }  
        ...
        logos.forEach(logo => {  
            ...
            img.onload = function () {  
                ...
                for (let i = 0; i < d.length; i += 4) {  
                    if (d[i + 3] > 0) {  
                        const bright = 0.299 * d[i] + 0.587 * d[i + 1] + 0.114 * d[i + 2];  
                        r += d[i]; g += d[i + 1]; b += d[i + 2]; cnt++;  
                        if (bright < 20) dark++;  
                    }  
                }  
                if (cnt > 0) {  
                    const avgBright = (0.299 * (r / cnt) + 0.587 * (g / cnt) + 0.114 * (b / cnt));  
                    if (avgBright < 25 && dark / cnt > 0.7) {  
                        const el = networkContainer.find(`img[alt="${logo.name}"]`);  
                        el.css({ 'filter': 'brightness(0) invert(1) contrast(1.2)', 'opacity': '0.95' });  
    ...
    function fillMetaInfo(render, data) {  
        ...
        const parts = [getMediaType(data)];  
        if (data.genres && data.genres.length) {  
            parts.push(...data.genres.slice(0, 2).map(g => Lampa.Utils.capitalizeFirstLetter(g.name)));  
        }  
        metaTextContainer.html(parts.join(' · '));  
    ...
    function fillAdditionalInfo(render, data) {  
        ...
        if (data.name) {  
            if (data.episode_run_time && data.episode_run_time.length) {  
                ...
                parts.push(`${avgRuntime} ${timeM}`);  
            }  
            const seasons = Lampa.Utils.countSeasons(data);  
            if (seasons) parts.push(formatSeasons(seasons));  
        } else {  
            if (data.runtime && data.runtime > 0) {  
                ...
                const timeM = Lampa.Lang.translate('time_m').replace('.', '');  
                parts.push(h > 0 ? `${h} ${timeH} ${m} ${timeM}` : `${m} ${timeM}`);  
            }  
        }  
        infoContainer.html(parts.join(' · '));  
    }  
    ...
    function waitForBackgroundLoad(activity, callback) {  
        ...
        if (typeof MutationObserver !== 'undefined') {  
            ...
            const iv = setInterval(() => {  
                if (background.hasClass('loaded')) { clearInterval(iv); setTimeout(complete, 100); }  
            }, 100);  
            setTimeout(() => { clearInterval(iv); if (!background.hasClass('applecation-animated')) complete(); }, 1500);  
        }  
    }  
    ...
    function fillRatings(ratingsContainer, data) {  
        ...
        if (imdb > 0) {  
            ratingsContainer.find('.rate--imdb div').text(parseFloat(imdb).toFixed(1));  
            ratingsContainer.find('.rate--imdb').removeClass('hide');  
        }  
        if (kp > 0) {  
            ratingsContainer.find('.rate--kp div').text(parseFloat(kp).toFixed(1));  
            ratingsContainer.find('.rate--kp').removeClass('hide');  
        }  
    }  
  
    // Визначення найкращої якості та української аудіодоріжки  
    function getBest(results) {  
        const best = { resolution: null, hdr: false, dolbyVision: false, audio: null, dub: false, ukr: false };  
        const resOrder = ['HD', 'FULL HD', '2K', '4K'];  
        const audioOrder = ['2.0', '4.0', '5.1', '7.1'];  
            ...
            if (item.ffprobe && Array.isArray(item.ffprobe)) {  
                item.ffprobe.forEach(stream => {  
                    ...
                    if (stream.codec_type === 'audio' && stream.channels) {  
                        const ch = parseInt(stream.channels);  
                        const aud = (ch >= 8) ? '7.1' : (ch >= 6) ? '5.1' : (ch >= 4) ? '4.0' : '2.0';  
    ...
    // Слайдшоу фонів (виправлено: ціль .full-start__background)  
    function startSlideshow($container, backdrops) {  
        if (!Lampa.Storage.get('applecation_slideshow', true) || backdrops.length < 2) return;  
        console.log('[NewCard] startSlideshow: backdrops', backdrops.length, 'container', $container.length ? $container[0].className : 'not found');  
        let idx = 0;  
        const interval = parseInt(Lampa.Storage.get('applecation_slideshow_time', '10000'), 10);  
        const quality = Lampa.Storage.get('applecation_slideshow_quality', 'w780');  
        stopSlideshow();  
        slideshowTimer = setInterval(() => {  
            idx = (idx + 1) % backdrops.length;  
            const url = Lampa.TMDB.image(`/t/p/${quality}${backdrops[idx].file_path}`);  
            const $cur = $container.find('img').first();  
            const $next = $(`<img src="${url}" style="opacity:0;position:absolute;top:0;left:0;width:100%;height:100%;object-fit:cover;">`);  
...
// Масштабування  
    function applyScales() {  
        const logoScale = parseInt(Lampa.Storage.get('applecation_logo_scale', '100'), 10);  
        const textScale = parseInt(Lampa.Storage.get('applecation_text_scale', '100'), 10);  
        const spacingScale = parseInt(Lampa.Storage.get('applecation_spacing_scale', '100'), 10);  
            ...
            <div class="full-start-new__right">  
                <div class="applecation__left">  
                    ...
                    <div class="applecation__content-wrapper">  
                        ...
                        <div class="applecation__quality-row"></div>  
                ...
                <div class="applecation__right">  
                    ...
                    <div class="rating--modss" style ation__ratings {  
                    margin-bottom: ${0.5 * spacingScale / 100}em !important;  
                }  
                .applecation .applecation__description {  
                    max-width: ${35 * textScale / 100}vw !important;  
...
/* Логотип */  
.applecation__logo { margin-bottom: 0.5em; opacity: 0; transform: translateY(20px); transition: transform 0.4s ease-out; will-change: transform; }  
.applecation__logo.loaded { opacity: 1; transform: translateY(0); }  
.applecation__logo img { display: block; max-width: 35vw; width: auto; height: auto; object-fit: contain; object-position: left center; max-height: 180px; }  
  
/* Мета */  
.applecation__meta { display: flex; align-items: center; color: #fff; font-size: 1.1em; margin-bottom: 0.5em; line-height: 1; opacity: 0; transform: translateY(15px); transition: opacity 0.3s ease-out; will-change: opacity; }  
...
.applecation__network { display: inline-flex; align-items: center; gap: 0.5em; line-height: 1; }  
.applecation__network img { display: block; max-height: 1.4em; width: auto; object-fit: contain; transition: filter 0.3s ease; }  
...
/* Рейтинги */  
.applecation__ratings { display: flex; align-items: center; gap: 0.8em; margin-bottom: 0.5em; opacity: 0; transform: translateY(15px); transition: opacity 0.4s ease-out, transform 0.4s ease-out; transition-delay: 0.08s; will-change: opacity, transform; }  
...
.applecation__ratings svg { width: 2.5em; height: auto; flex-shrink: 0; color: rgba(255,255,255,0.85); }  
.applecation__ratings .rate--kp svg { width: 1.5em; }  
.applecation__ratings > div > div { font-size: 0.95em; font-weight: 600; line-height: 1; color: #fff; }  
body.applecation--hide-ratings .applecation__ratings { display: none !important; }  
body.applecation--ratings-corner .applecation__right { gap: 1em; }  
body.applecation--ratings-corner .applecation__ratings { margin-bottom: 0; }  
  
/* Опис */  
.applecation__description { color: rgba(255,255,255,0.6); font-size: 0.95em; line-height: 1.5; margin-bottom: 0.5em; max-width: 35vw; display: -webkit-box; -webkit-line-clamp: 4; -webkit-box-orient: vertical; overflow: hidden; text-overflow: ellipsis; opacity: 0; transform: translateY(15px); transition: opacity 0.4s ease-out, transform 0.4s ease-out; transition-delay: 0.1s; will-change: opacity, transform; }  
.applecation__description.show { opacity: 1; transform: translateY(0); }  
  
/* Додаткова інформація */  
.applecation__info { color: rgba(255,255,255,0.75); font-size: 1em; line-height: 1.4; margin-bottom: 0.5em; opacity: 0; transform: translateY(15px); transition: opacity 0.4s ease-out, transform 0.4s ease-out; transition-delay: 0.15s; will-change: opacity, transform; }  
.applecation__info.show { opacity: 1; transform: translateY(0); }  
  
/* Якість */  
.applecation__quality-row { display: flex; justify-content: flex-start; align-items: center; gap: 8px; margin-bottom: 0.5em; opacity: 0; transform: translateY(10px); transition: opacity 0.4s ease, transform 0.4s ease; }  
.applecation__quality-row.show { opacity: 1; transform: translateY(0); }  
.quality-item img { height: 1.4em; width: auto; filter: drop-shadow(0 0 1px rgba(255,255,255,0.4)); }  
...
.applecation .full-start__status { display: none; }  
    ...
    // Завантаження логотипа та якості + слайдшоу  
    ...
    $.get(apiUrl, imagesData => {  
        ...
        // Слайдшоу (ціль .full-start__background)  
        if (imagesData.backdrops && imagesData.backdrops.length > 1) {  
            const $bg = render.find('.full-start__background');  
            if ($bg.length) startSlideshow($bg, imagesData.backdrops.slice(0, 15));  
            else startSlideshow(render.find('.full-start-new__poster'), imagesData.backdrops.slice(0, 15));  
        }  
        // Якість та український дубляж  
        if (Lampa.Storage.get('applecation_quality', true) && Lampa.Parser && Lampa.Parser.get) {  
            Lampa.Parser.get({ search: data.title || data.name, movie: data, page: 1 }, res => {  
                if (res && res.Results) {  
                    const best = getBest(res.Results);  
                    const list = [];  
                    ...
                    const qRow = render.find('.applecation__quality-row').empty();  
                    list.forEach((type, i) => {  
                        if (svgIcons[type]) {  
                            const $q = $(`<div class="quality-item" style="animation-delay:${i*0.1}s"><img src="${svgIcons[type]}"></div>`);  
                            qRow.append($q);  
                        }  
                    });  
                    waitForBackgroundLoad(activity, () => { qRow.addClass('show'); });  
                }  
            });  
        }  
    }).fail(() => {  
...
const translations = {  
    ...
    slideshow: { uk: 'Слайдшоу фону' },  
    slideshow_desc: { uk: 'Автоматична зміна зображень фону' },  
    slideshow_time: { uk: 'Інтервал слайдшоу' },  
    slideshow_quality: { uk: 'Якість слайдшоу' },  
    quality: { uk: 'Значки якості' },  
    quality_desc: { uk: 'Показувати 4K/HDR/Audio/UKR' }  
};  
function t(key) { return translations[key]?.uk || '???'; }  
  
// Керування станом  
function updateZoomState() {  
    const enabled = Lampa.Storage.get('applecation_apple_zoom', true);  
    $('body').toggleClass('applecation--zoom-enabled', enabled);  
}  
function updateLogoColors() { /* порожньо */ }  
...
function addSettings() {  
    ...
    Lampa.SettingsApi.addParam({  
        ...
        onChange: v => $('body').toggleClass('applecation--hide-ratings', !v)  
    });  
  
    Lampa.SettingsApi.addParam({  
        component: 'applecation_settings',  
        param: { name: 'applecation_ratings_position', type: 'select', values: { card: t('position_card'), corner: t('position_corner') }, default: 'card' },  
        field: { name: t('ratings_position'), description: t('ratings_position_desc') },  
        onChange: v => {  
            Lampa.Storage.set('applecation_ratings_position', v);  
            $('body').removeClass('applecation--ratings-card applecation--ratings-corner').addClass('applecation--ratings-' + v);  
            addCustomTemplate();  
        }  
    });  
  
    ['logo', 'text', 'spacing'].forEach(type => {  
        Lampa.SettingsApi.addParam({  
            component: 'applecation_settings',  
            param: { name: `applecation_${type}_scale`, type: 'select', values: { '70':'70%','80':'80%','90':'90%','100':t('scale_default'),'110':'110%','120':'120%','130':'130%','140':'140%','150':'150%','160':'160%' }, default: '100' },  
            field: { name: t(`${type}_scale`), description: t(`${type}_scale_desc`) },  
            onChange: () => { Lampa.Storage.set(`applecation_${type}_scale`, Lampa.Storage.get(`applecation_${type}_scale`)); applyScales(); }  
        });  
    });  
    ...
    Lampa.SettingsApi.addParam({  
        ...
        field: { name: t('slideshow'), description: t('slideshow_desc') }  
    });  
    Lampa.SettingsApi.addParam({  
        component: 'applecation_settings',  
        param: { name: 'applecation_slideshow_time', type: 'select', values: { '10000':'10 с','15000':'15 с','20000':'20 с' }, default: '10000' },  
        field: { name: t('slideshow_time') }  
    });  
    Lampa.SettingsApi.addParam({  
        component: 'applecation_settings',  
        param: { name: 'applecation_slideshow_quality', type: 'select', values: { 'w300':'w300','w780':'w780','w1280':'w1280' }, default: 'w780' },  
        field: { name: t('slideshow_quality') }  
    });  
    ...
    Lampa.SettingsApi.addParam({  
        ...
        field: { name: t('quality'), description: t('quality_desc') }  
    });  
...
function initializePlugin() {  
    ...
    if (!Lampa.Platform.screen('tv')) { console.log('NewCard', 'TV mode only'); return; }  
    patchApiImg();  
    addCustomTemplate();  
    addStyles();  
    addSettings();  
...
function registerPlugin() {  
    const manifest = { type: 'other', version: '1.1.0', name: 'NewCard', description: 'Новий дизайн картки фільму/серіалу.', author: '', icon: PLUGIN_ICON };  
    if (Lampa.Manifest) {  
        if (!Lampa.Manifest.plugins) Lampa.Manifest.plugins = {};  
        if (Array.isArray(Lampa.Manifest.plugins)) Lampa.Manifest.plugins.push(manifest);  
        else Lampa.Manifest.plugins.newcard = manifest;  
    }  
}  
