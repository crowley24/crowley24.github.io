(function () {  
    ...
    const svgIcons = {
        ...
        'Dolby Vision': 'https://lampatv.github.io/img/quality/dv.svg',
    ...
    const translations = {  
        ...
        slideshow_quality_desc: { uk: 'Роздільна здатність фонових зображень' }
    };  
    ...
    function addSettings() {  
        ...
        Lampa.SettingsApi.addParam({
            ...
            field: { name: 'Значки якості (4K, HDR, UKR)', description: 'Відображати знайдену якість через парсер' }
        });

        Lampa.SettingsApi.addParam({
            component: 'applecation_settings',
            param: { name: 'applecation_slideshow_quality', type: 'select', values: { 'w300': 'Низька (w300)', 'w780': 'Середня (w780)', 'w1280': 'Висока (w1280)', 'original': 'Оригінальна' }, default: 'w780' },
            field: { name: t('slideshow_quality'), description: t('slideshow_quality_desc') }
        });
...
.applecation__meta-left{display:flex;align-items:center;line-height:1;flex-wrap:wrap;gap:0.5em;}  
...
.applecation__meta-text{margin-left:0.5em;line-height:1}  
.applecation__quality-icons{display:flex;gap:5px;margin-left:10px;align-items:center;}
...
function startSlideshow($poster, backdrops) {
    if (!Lampa.Storage.get('applecation_apple_zoom', true) || backdrops.length < 2) return;
    var index = 0;
    var quality = Lampa.Storage.get('applecation_slideshow_quality', 'w780');
    ...
    slideshowTimer = setInterval(function() {
        ...
        nextImg.onload = function() {
            var $newImg = $('<img src="' + imgUrl + '" class="full-start__background loaded applecation-animated" style="opacity: 0; z-index: -1;">');
...
function getBest(items) {
    let best = { resolution: '', hdr: false, dolbyVision: false, ukr: false, dub: false, audio: '' };
    items.forEach(item => {
        if (item.quality && item.quality.indexOf('2160') > -1) best.resolution = '4K';
        if (item.quality && item.quality.indexOf('HDR') > -1) best.hdr = true;
        if (item.quality && item.quality.indexOf('DV') > -1) best.dolbyVision = true;
        if (item.voice && (item.voice.indexOf('Укр') > -1 || item.voice.indexOf('UKR') > -1)) best.ukr = true;
        if (item.voice && item.voice.indexOf('Дубляж') > -1) best.dub = true;
    });
    return best;
}
...
function loadLogo(event) {  
    ...
    const qualityIconsContainer = render.find('.applecation__quality-icons');
    ...
    // Нова функція: Значки якості
    if (Lampa.Storage.get('applecation_show_quality_icons') && Lampa.Parser.get) {
        Lampa.Parser.get({ search: data.title || data.name, movie: data, page: 1 }, function(res) {
            if (res && res.Results) {
                var best = getBest(res.Results);
                qualityIconsContainer.empty();
                if (best.resolution === '4K') qualityIconsContainer.append(`<img src="${svgIcons['4K']}">`);
                if (best.dolbyVision) qualityIconsContainer.append(`<img src="${svgIcons['Dolby Vision']}">`);
                else if (best.hdr) qualityIconsContainer.append(`<img src="${svgIcons['HDR']}">`);
                if (best.ukr) qualityIconsContainer.append(`<img src="${svgIcons['UKR']}">`);
                if (best.dub) qualityIconsContainer.append(`<img src="${svgIcons['DUB']}">`);
            }
        });
    }
    ...
    if (logoCache.has(cacheKey)) {  
        ...
        renderBestLogo(cached, logoContainer, titleElement, activity);  
        if (cached.backdrops) startSlideshow(activity.render().find('.full-start__poster'), cached.backdrops);
        return;  
    }  
    ...
    $.get(apiUrl, (imagesData) => {  
        ...
        if (Lampa.Activity.active()?.component === 'full') {
            ...
            if (imagesData.backdrops) startSlideshow(activity.render().find('.full-start__poster'), imagesData.backdrops);
        }
    }).fail(() => {  
        titleElement.show();  
        waitForBackgroundLoad(activity, () => { logoContainer.addClass('loaded'); });  
