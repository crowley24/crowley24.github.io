(function () {
    ...
    function applyStyles() {
        ...
        css += '@keyframes qb_in { from { opacity: 0; transform: translateX(-20px); } to { opacity: 1; transform: translateX(0); } } ';
        
        /* Контейнер фону на весь екран */
        ...
        css += '.full-start-new__poster img { position: absolute; top: 0; left: 0; width: 100%; height: 100%; object-fit: cover; opacity: 0; transition: opacity 2s ease-in-out !important; ';
        css += (isAnimationEnabled ? 'animation: kenBurnsEffect 40s ease-in-out infinite !important; ' : '');
        css += 'mask-image: linear-gradient(to right, #000 20%, transparent 95%), linear-gradient(to top, #000 30%, transparent 100%) !important; ';
        css += '-webkit-mask-image: linear-gradient(to right, #000 20%, transparent 95%), linear-gradient(to top, #000 30%, transparent 100%) !important; } ';
        
        /* Приховуємо стандартний блюр Lampa */
        css += '.full-start-new__bg { display: none !important; } ';

        /* Текстовий контент у стилі Netflix (зліва внизу) */
        css += '.full-start-new__right { background: none !important; position: absolute !important; bottom: 8% !important; left: 5% !important; width: 50% !important; z-index: 10 !important; display: flex !important; flex-direction: column !important; align-items: flex-start !important; padding: 0 !important; } ';
        
        /* Логотип фільму */
        css += '.full-start-new__title { width: 100%; display: flex; justify-content: flex-start; margin-bottom: 20px; } ';
        css += '.full-start-new__title img { max-width: 500px; max-height: 180px; object-fit: contain; filter: drop-shadow(0 0 15px rgba(0,0,0,0.8)); } ';
        
        /* Опис та теглайн */
        css += '.full-start-new__tagline { font-size: 1.5em !important; font-weight: 300 !important; color: #fff !important; text-shadow: 2px 2px 4px rgba(0,0,0,0.9) !important; margin: 10px 0 !important; text-align: left !important; } ';
        css += '.full-start-new__description { font-size: 1.2em !important; line-height: 1.4 !important; max-width: 80% !important; text-shadow: 1px 1px 3px rgba(0,0,0,1) !important; color: #eee !important; } ';

        /* Блок іконок та студій */
        css += '.plugin-info-block { display: flex; flex-direction: column; align-items: flex-start; gap: 20px; margin-top: 25px; width: 100%; } ';
        css += '.studio-row, .quality-row { display: flex; align-items: center; flex-wrap: wrap; gap: 15px; } ';
        
        css += '.studio-item { height: 45px; opacity: 0; animation: qb_in 0.6s ease forwards; padding: 8px 16px; border-radius: 12px; display: flex; align-items: center; justify-content: center; ';
        if (bgOpacity !== '0') {
        ...
        css += '.quality-item { height: 32px; opacity: 0; animation: qb_in 0.6s ease forwards; filter: drop-shadow(0 0 5px rgba(0,0,0,0.5)); } ';
    ...
    function startSlideshow($poster, backdrops) {
        ...
        slideshowTimer = setInterval(function() {
            ...
            var $currentImg = $poster.find('img').last();
            ...
            $newImg.on('load', function() {
                ...
                setTimeout(function() {
                    $poster.find('img').not($newImg).remove();
                }, 2100);
            });
        }, interval);
    }
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        ...
                        if (res.backdrops && res.backdrops.length > 0) {
                            $poster.html('<img src="' + Lampa.TMDB.image('/t/p/original' + res.backdrops[0].file_path) + '" style="opacity: 1;">');
    ...
    function addSettings() {
        Lampa.SettingsApi.addComponent({
            ...
            name: 'TV Інтерфейс (Netflix Style)'
        });

        Lampa.SettingsApi.addParam({
            component: 'tv_interface',
            param: { name: 'tv_interface_animation', type: 'trigger', default: true },
            field: { name: 'Анімація Ken Burns', description: 'Ефект наближення фону' },
        ...
        Lampa.SettingsApi.addParam({
            ...
            field: { name: 'Слайд-шоу фону', description: 'Зміна кадрів фільму на фоні' }
        });
