...
function startSlideshow(render, backdrops, currentLang) {  
    ...
    var $bgContainer = render.find('.full-start__background');
    if (!$bgContainer.length) return;

    // Підганяємо стилі контейнера та створюємо шар для плавного переходу
    $bgContainer.css({ 'position': 'relative', 'overflow': 'hidden' });
    
    var $imgActive = $bgContainer.find('img').first();
    var $imgNext = $bgContainer.find('.cas-bg-next');

    if (!$imgNext.length) {
        $imgNext = $('<img class="cas-bg-next">').appendTo($bgContainer);
    }

    // Задаємо апаратне прискорення для обох зображень
    var commonStyles = {
        'position': 'absolute',
        'top': 0,
        'left': 0,
    ...
    $imgActive.css(commonStyles).css('opacity', 1);
    $imgNext.css(commonStyles).css('opacity', 0);
    ...
    currentInterval = setInterval(function () {  
        ...
        // Використовуємо w1280 замість original для зменшення навантаження на GPU TV
        var nextSrc = Lampa.TMDB.image('/t/p/w1280' + final_backdrops[idx].file_path);  

        var img = new Image();  
        img.onload = function () {  
            $imgNext.attr('src', nextSrc);

            // М'який кросфейд через requestAnimationFrame
            requestAnimationFrame(function () {
                $imgNext.css('opacity', 1);
                $imgActive.css('opacity', 0);

                setTimeout(function () {
                    // Міняємо посилання та шари місцями
                    $imgActive.attr('src', nextSrc).css('opacity', 1);
