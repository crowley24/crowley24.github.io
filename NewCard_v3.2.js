(function () {
    ...
    function applyStyles() {
        ...
        var css = '';
        // Анімація Ken Burns для фону
        css += '@keyframes kenBurnsBackground { 0% { transform: scale(1); } 50% { transform: scale(1.15); } 100% { transform: scale(1); } } ';
        css += '@keyframes ui_reveal { from { opacity: 0; transform: translateY(15px); } to { opacity: 1; transform: translateY(0); } } ';
        
        // Приховуємо оригінальний постер повністю
        ...
        // Налаштування фонового шару Lampa
        css += '.background { background: #000 !important; overflow: hidden !important; } ';
        css += '.background__image { ';
        css += (isPosterAnim ? 'animation: kenBurnsBackground 30s ease-in-out infinite !important; ' : '');
        css += 'transition: opacity 1.5s ease-in-out !important; position: absolute; width: 100% !important; height: 100% !important; object-fit: cover !important; ';
        css += 'mask-image: linear-gradient(to bottom, #000 0%, #000 50%, transparent 100%) !important; -webkit-mask-image: linear-gradient(to bottom, #000 0%, #000 50%, transparent 100%) !important; } ';
        
        // Блок з інформацією (зсунутий для ТВ)
        css += '.full-start-new__right { background: none !important; margin-top: 10vh !important; z-index: 2 !important; display: flex !important; flex-direction: column !important; align-items: center !important; padding: 0 10px !important; gap: ' + blocksGap + ' !important; } ';
        ...
        css += '.full-start-new .full-start__button.focus { background: rgba(255,255,255,0.1) !important; border-radius: 10px; } ';
    ...
    /**
     * СЛАЙДШОУ ТА ЛОГО (ТЕПЕР ДЛЯ ФОНУ)
     */
    ...
    function startPosterSlideshow($bgContainer, items) {
    ...
    function setupSettings() {
        ...
        Lampa.SettingsApi.addParam({ component: 'mobile_interface', param: { name: 'mobile_interface_ui_anim', type: 'trigger', default: true }, field: { name: 'Анімація тексту' }, onChange: applyStyles });
        ...
        Lampa.SettingsApi.addParam({ component: 'mobile_interface', param: { name: 'mobile_interface_logo_size_v2', type: 'select', values: { '125': 'Малий', '150': 'Середній', '180': 'Стандарт', '210': 'Великий' }, default: '125' }, field: { name: 'Висота логотипу' }, onChange: applyStyles });
        ...
        Lampa.SettingsApi.addParam({ component: 'mobile_interface', param: { name: 'mobile_interface_quality', type: 'trigger', default: true }, field: { name: 'Визначати якість' } });
    }
