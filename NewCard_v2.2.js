(function () {    
  ...
  :root {   
      ...
      --cas-blocks-gap: 30px;   
      --cas-meta-size: 1.3em;   
      --cas-anim-curve: cubic-bezier(0.4, 0, 0.2, 1);   
  }  
    
  /* Преміальна анімація фону */  
  @keyframes casKenBurns {  
      ...
      50% {   
          transform: scale(1.08) translateZ(0) rotate(0deg);   
  ...
  body.cas--zoom-enabled .full-start__background.loaded {   
      animation: casKenBurns 60s ease-in-out infinite !important;   
  }  
  ...
  .left-title .full-start-new__buttons .full-start__button::before {  
      content: '';  
      position: absolute;  
      inset: 0;  
      border-radius: 12px;  
  ...
  .left-title .full-start-new__buttons .full-start__button.focus::before {  
      opacity: 1;  
  }  
  ...
  .left-title__content.cas-animated .cas-logo { opacity: 1; transform: translateY(0); transition-delay: 0s; }    
  .left-title__content.cas-animated .cas-ratings-line { opacity: 1; transform: translateY(0); transition-delay: 0.1s; }    
  .left-title__content.cas-animated .cas-description { opacity: 1; transform: translateY(0); transition-delay: 0.2s; }    
  .left-title__content.cas-animated .cas-studios-row { opacity: 1; transform: translateY(0); transition-delay: 0.3s; }    
  .left-title__content.cas-animated .full-start-new__buttons { opacity: 1; transform: translateY(0); transition-delay: 0.4s; }    
  ...
  .cas-description { color: rgba(255, 255, 255, 0.7); font-size: 0.95em; line-height: 1.5; max-width: 35vw; display: -webkit-box; -webkit-line-clamp: 4; -webkit-box-orient: vertical; overflow: hidden; }    
              ...
              if (data && data.id) {    
                  ...
                  const processImages = (res) => {    
                      ...
                      if (Lampa.Storage.get('cas_slideshow_enabled') && res.backdrops?.length > 1) {    
                          // Попередньо завантажуємо наступні зображення    
                          const preloadedImages = [];    
                          const maxImages = Math.min(res.backdrops.length, 10);      
                            
                          for (let i = 0; i < maxImages; i++) {    
                              const img = new Image();    
                              img.src = Lampa.TMDB.image('/t/p/original' + res.backdrops[i].file_path);    
                          ...
                          window.casBgInterval = setInterval(() => {    
                              ...
                              if (!bg.length || !document.body.contains(bg[0])) return stopSlideshow();      
                                
                              isChanging = true;    
                              idx = (idx + 1) % maxImages;      
                                
                              // Плавна зміна зображення з попереднім завантаженням    
                              const nextImg = preloadedImages[idx];    
                              if (nextImg && nextImg.complete) {    
                                  bg.css('opacity', '0');    
                                  setTimeout(() => {    
                                      bg.attr('src', nextImg.src);    
                                      bg.css('opacity', '1');    
                                      isChanging = false;    
                                  }, 300);    
                              } else {    
                          ...
                          }, 20000); // Збільшено інтервал для зменшення навантаження    
                      }    
                  };      
