(function () {  
  ...
  var Player = /*#__PURE__*/function () {  
    ...
    _createClass(Player, [{  
      ...
      value: function show() {  
        ...
        if (mode === 'pip') {  
          // Застосовуємо розмір для PiP режиму  
  ...
  function lisen(node, k) {  
    return kthAncestor.call(this, node, k);  
  }  
  
  function binaryLifting(root, connections) {  
    this.connections = connections;  
    this.up = new Map();  
    this.log = Math.floor(Math.log2(connections.size)) + 1;  
    dfs.call(this, root, null);  
    return {  
      lisen: lisen.bind(this)  
    };  
  }  
  ...
  var FrequencyMap = /*#__PURE__*/function () {  
    ...
    _createClass(FrequencyMap, [{  
      ...
      value: function insert(node) {  
        ...
        this.frequencyMap.get(frequency).add(node);  
  
        if (frequency < this.minFrequency || this.minFrequency === 0) {  
      ...
      value: function refresh(node) {  
        ...
        if (this.frequencyMap.has(frequency - 1)) {  
          this.frequencyMap.get(frequency - 1)["delete"](node);  
  
          if (this.frequencyMap.get(frequency - 1).size === 0 && frequency - 1 === this.minFrequency) {  
            this.minFrequency = frequency;  
          }  
        }  
  ...
  var CacheNode = /*#__PURE__*/function () {  
    ...
    _createClass(CacheNode, [{  
      key: "updateFrequency",  
      value: function updateFrequency() {  
        this.frequency++;  
      }  
    }]);  
