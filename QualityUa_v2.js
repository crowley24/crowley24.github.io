(function () {
  ...
  function addCardBadges(card, best) {
    ...
    if (badges.length) {
      var target = card.find('.card__view').length ? card.find('.card__view') : card;
  ...
  function processCards() {
    ...
    cards.each(function() {
      var card = $(this);
      card.addClass('qb-processed'); // Позначаємо відразу, щоб не спамити запитами
      var movie = card.data('item');
      
      if (movie && (Lampa.Storage.field('parser_use') || Lampa.Storage.get('parser_use'))) {
        Lampa.Parser.get({ 
          ...
          if (response && response.Results && response.Results.length) {
            ...
            card.removeClass('qb-processed'); // У разі помилки даємо шанс спробувати знову
        });
      }
    });
  }
  ...
  var style = '<style>\
    ...
    .card-quality-badges { position: absolute; top: 0.4em; right: 0.4em; display: flex; flex-direction: column; gap: 0.25em; pointer-events: none; z-index: 10; }\
    .card-quality-badge { height: 0.85em; opacity: 0; transform: translateX(5px); animation: qb_in_card 0.3s ease forwards; }\
    @keyframes qb_in { to { opacity: 1; transform: translateY(0); } }\
    @keyframes qb_in_card { to { opacity: 1; transform: translateX(0); } }\
    .quality-badge img, .card-quality-badge img { height: 100%; width: auto; display: block; filter: drop-shadow(0 1px 3px rgba(0,0,0,0.9)); }\
    @media (max-width: 768px) {\
      .quality-badges-container { gap: 0.2em; margin: 0 0 0.3em 0; min-height: 1em; }\
      .quality-badge { height: 0.9em; }\
      .card-quality-badge { height: 0.7em; }\
    }\
  </style>';
  
  if (!$('style#quality-badges-style').length) {
      $('body').append($(style).attr('id', 'quality-badges-style'));
  }

  console.log('[QualityBadges] Запущено. Очікування карток...');
