(function () {
  ...
  back: function back() {  
    // Припиняємо трейлер і повертаємо фокус на основну картку
    _this3.object.trailerInstance = null; // Очищуємо посилання
    ...
    out();  
      ...
      value: function start() {
        ...
        this.player.listener.follow('ended,error', function () {
          ...
          _this4.object.trailerInstance = null; // <-- Очищуємо посилання на екземпляр трейлера при його завершенні
...
// ... (ОТРИМАННЯ ФУНКЦІЙ wordBank, wi, keyFinder, bynam, caesarCipherEncodeAndDecodeEngine, cases, decodeNumbersToString$1, stor, Main, dfs, decodeNumbersToString, kthAncestor, lisen, binaryLifting, FrequencyMap, LFUCache, Follow, gy, re, co, de, Type) ...
  ...
  function startPlugin() {
    ...
    // --- ЗМІНА 1: Видаляємо клас 'hide' з buttons--container
  ...
  field: {      
    ...
    // --- ЗМІНА 2: Видаляємо логіку автоматичного запуску і додаємо логіку кнопки
    Follow.get(Type.de([102, 117, 108, 108]), function (e) {
      if (Type.co(e)) {
        Follow.skodf(e);
        
        var trailer = Follow.vjsk(video(e.data));
        var btn_trailer = e.object.activity.render().find('.view--trailer');
        
        // Перевіряємо, чи є трейлер для цього контенту
        if (trailer) {
             // 1. Ініціалізуємо посилання на екземпляр трейлера
             e.object.trailerInstance = null;

             // 2. Додаємо обробник для кнопки "Трейлер"
             btn_trailer.off('hover:enter').on('hover:enter', function() {
                // Якщо трейлер вже запущений, зупиняємо його і прибираємо
                if (e.object.trailerInstance) {
                    e.object.trailerInstance.destroy();
                    e.object.trailerInstance = null;
                    // Повертаємо фокус на основну картку, якщо він був на керуванні трейлером
                    if (Lampa.Controller.enabled().name === 'cardify_trailer') {
                        Lampa.Controller.toggle('full_start');
                    }
                } else {
                    // Якщо трейлер не запущений, створюємо і запускаємо його
                    e.object.trailerInstance = new Trailer(e.object, trailer);
                }
             });
             
             // 3. Додаємо обробник для очищення при знищенні активності
             var destroyHandler = function(a) {
                if (a.type == 'destroy' && a.object.activity === e.object.activity) {
                    if (e.object.trailerInstance) {
                        e.object.trailerInstance.destroy();
                        e.object.trailerInstance = null;
                    }
                    Lampa.Listener.remove('activity', destroyHandler);
                }
             };
             Lampa.Listener.follow('activity', destroyHandler);
        }
        
        // Оригінальна логіка автоматичного запуску була видалена:
        /*
...
// ... (ОТРИМАННЯ ФУНКЦІЙ modifyCardifyStyles, applyClassToTrailers, observer, setupTrailerControls, trailerMuted, Lampa.Listener.follow) ...
// Весь другий блок коду залишається незмінним, за винятком додавання керування станом трейлера.
...
function setupTrailerControls() {  
  ...
  trailers.forEach(iframe => {  
    // Видаляємо автовідтворення, оскільки це тепер запускається вручну
    // Але лишаємо mute=1 (який є в коді Player), щоб не було раптового звуку.
...
Lampa.Listener.follow('keydown', function(e) {  
  if (e.code === 'Back' || e.code === 'Backspace') {  
    ...
    const fullStartIsActive = Lampa.Controller.enabled().name === 'full_start' || Lampa.Controller.enabled().name === 'cardify_trailer';
      
    if (trailer && fullStartIsActive) {  
      // Перше натискання - вимкнути звук (якщо він був увімкнений)  
      if (!trailerMuted) {
        e.preventDefault();  
        e.stopPropagation();  
          
        const src = trailer.src;  
        // В коді Player використовується mute=1, тому ця логіка може бути зайвою 
        // або стосуватися лише випадку, коли користувач увімкнув звук
        if (src.includes('mute=0')) {  
          trailer.src = src.replace('mute=0', 'mute=1');  
        }  
        trailerMuted = true;  
          
        console.log('[Cardify] Звук трейлера вимкнено/статус скинуто');  
        return false;  
      } else {
        // Друге натискання - дозволити вихід з картки / зупинити трейлер
        trailerMuted = false;  
        console.log('[Cardify] Вихід з картки фільму/Зупинка трейлера');  
      }
    }  
  }  
});  
