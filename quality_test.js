(function () {
    ...
    // I. КОНФІГУРАЦІЯ
    // =======================================================
    var Q_LOGGING = false; // Залишаємо вимкненим
     ...
     * (ОНОВЛЕНО: Посилено регулярні вирази для більшої надійності)
     */
    function getDisplayQuality(numericQuality, torrentTitle) {
        ...
        // 1. Визначення HDR/DV
        // Шукаємо: dolby vision, dv, dovi, hdr10+, hdr10, hdr
        ...
        // 2. Визначення роздільної здатності (Без змін)
        ...
        // Логіка корекції: Якщо є DV/HDR, має бути 4K (якщо не знайдено іншої)
        if (result.hdr && !result.quality) {
            result.quality = '4K';
        }
        
        // Логіка: не показуємо DV/HDR, якщо роздільна здатність HD
    ...
    // ... (getBestReleaseFromJacred та кешування без змін, оскільки вони коректно передають дані) ...
    
    /**
     * Функція застосування якості до картки.
     * (ОНОВЛЕНО: Логіка застосування HDR/DV)
     */
    function applyQualityToCard(card, quality, hdr, source, qCacheKey) {
        ...
        if (quality === NO_RESULT_MARKER) {
             if (source === 'JacRed') {
                 // Важливо: кешуємо саме з маркером
    ...
    // ... (Інші функції та ініціалізація без змін) ...
    
    // Повністю переписаний, оновлений та стабілізований код:
    
    // ======================================================================================================================================
    // КОМПЛЕКСНИЙ КОД (ВКЛЮЧАЮЧИ ВИПРАВЛЕННЯ DV/HDR, СТАБІЛІЗАЦІЮ ТА СТИЛІ)
    // ======================================================================================================================================
    ...
    (function () {
        ...
        function getDisplayQuality(numericQuality, torrentTitle) {
            ...
            // 1. Визначення HDR/DV (ПОСИЛЕНА ЛОГІКА)
            if (/\b(dolby\s*vision|dolbyvision|dovi|dv)\b/i.test(title)) {
                result.hdr = 'DV';
            } else if (/\b(hdr10\+|hdr10|hdr)\b/i.test(title)) {
                result.hdr = 'HDR';
