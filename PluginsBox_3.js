...
Lampa.SettingsApi.addParam({
    ...
    field: {
        ...
        description: 'Плагін дозволяє обрати доповнення (скрінсейвери, стиль кнопок, стиль плеєра тощо)'
    },
    onChange: function(value) {
        if (value == '1') {
        ...
        if (value == '2') {
            deletePlugin("https://andreyurl54.github.io/diesel5/tricks.js");
        }
    },
    onRender: function (item) {
        $('.settings-param__name', item).css('color','#f3d900');
        ...
        setTimeout(function() {
            var container = $('div[data-name="Tricks"]');
            if (container.length && container.find('.settings-param__status').length === 0) {
                container.append('<div class="settings-param__status one"></div>');
            }
            var pluginStatus = null;
            for (var i = 0; i < pluginsArray.length; i++) {
                if (pluginsArray[i] && pluginsArray[i].url === 'https://andreyurl54.github.io/diesel5/tricks.js') {
                    pluginStatus = pluginsArray[i].status;
                    break;
                }
            }
