(function () {    
    ...
    function addTranslations() {    
        ...
        Lampa.Lang.add({    
            ...
            tmdb_mod_view_label: { ru: "Тип постерів", uk: "Тип постерів" },  
            tmdb_mod_shuffle_label: { ru: "Перемішувати картки", uk: "Перемішувати картки" },  
            tmdb_mod_view_vertical: { ru: "Вертикальні", uk: "Вертикальні" },  
            tmdb_mod_view_horizontal: { ru: "Горизонтальні", uk: "Горизонтальні" },  
    ...
    var createDiscoveryMain = function (parent) {    
        return function () {    
            ...
            // Сортування добірок за порядком  
            var sortedConfig = collectionsConfig.slice().sort(function(a, b) {  
                var orderA = Lampa.Storage.get('tmdb_mod_' + a.id + '_order', a.defaultOrder);  
                var orderB = Lampa.Storage.get('tmdb_mod_' + b.id + '_order', b.defaultOrder);  
    ...
    function addSettings() {      
        ...
        collectionsConfig.forEach(function(cfg) {      
            ...
            Lampa.SettingsApi.addParam({  
                component: 'tmdb_mod',  
                param: {   
                    name: 'tmdb_mod_' + cfg.id + '_order',   
                    type: 'input',  
                    default: cfg.defaultOrder.toString()  
                },  
                field: {   
                    name: Lampa.Lang.translate('tmdb_mod_order_label') + ': ' + translatedName,  
                    description: 'Введіть число для порядку відображення (менше число = вище)'  
                },  
                onChange: function (value) {  
            ...
            // Тип постерів (вертикальні/горизонтальні)  
            Lampa.SettingsApi.addParam({  
                component: 'tmdb_mod',  
                param: {   
                    name: 'tmdb_mod_' + cfg.id + '_view',   
                    type: 'select',  
                    values: {   
                        vertical: Lampa.Lang.translate('tmdb_mod_view_vertical'),   
                        horizontal: Lampa.Lang.translate('tmdb_mod_view_horizontal')   
                    },  
                    default: 'vertical'  
                },  
                field: {   
                    name: Lampa.Lang.translate('tmdb_mod_view_label') + ': ' + translatedName,  
                    description: 'Оберіть орієнтацію постерів для цієї добірки'  
                },  
                onChange: function (value) {  
                    Lampa.Storage.set('tmdb_mod_' + cfg.id + '_view', value);  
                    Lampa.Noty.show(Lampa.Lang.translate('tmdb_mod_noty_reload'));  
                }  
            });  
            ...
            Lampa.SettingsApi.addParam({  
                component: 'tmdb_mod',  
                param: {   
                    name: 'tmdb_mod_' + cfg.id + '_shuffle',   
                    type: 'trigger',   
                    default: false   
                },  
                field: {   
                    name: Lampa.Lang.translate('tmdb_mod_shuffle_label') + ': ' + translatedName,  
                    description: 'Рандомізувати порядок карток при кожному завантаженні'  
                },  
                onChange: function (value) {  
    ...
    function initPlugin() {    
        try {    
            ...
            console.log('[TMDB_MOD] Плагін успішно ініціалізовано з розширеними налаштуваннями');  
            return true;    
        } catch (e) {    
