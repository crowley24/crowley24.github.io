...
// Оновлені стилі для трьохколонкового інтерфейсу  
Lampa.Template.add(plugin.component + '_style',   
'<style>' +  
'.PLUGIN__container { display: flex; height: 100%; }' +  
'.PLUGIN__groups { width: 25%; flex-shrink: 0; overflow-y: auto; border-right: 1px solid #333; }' +  
'.PLUGIN__channels { width: 45%; flex-shrink: 0; overflow-y: auto; }' +  
'.PLUGIN__epg { width: 30%; flex-shrink: 0; overflow-y: auto; border-left: 1px solid #333; padding: 1em; }' +  
'.group-item { padding: 0.8em; border-bottom: 1px solid #333; cursor: pointer; }' +  
'.group-item.focus { background: #444; }' +  
'.group-item.selected { background: #0066cc; }' +  
...
'.PLUGIN div.card__view{position:relative;background-color:#353535;background-color:#353535a6;border-radius:1em;cursor:pointer;padding-bottom:60%}' +  
'.PLUGIN.square_icons div.card__view{padding-bottom:100%}' +  
'.PLUGIN img.card__img,.PLUGIN div.card__img{background-color:unset;border-radius:unset;max-height:100%;max-width:100%;height:auto;width:auto;position:absolute;top:50%;left:50%;-moz-transform:translate(-50%,-50%);-webkit-transform:translate(-50%,-50%);transform:translate(-50%,-50%);font-size:2em}' +  
'.PLUGIN.contain_icons img.card__img{height:95%;width:95%;object-fit:contain}' +  
...
'.PLUGIN-program{display:-webkit-box;display:-webkit-flex;display:-moz-box;display:-ms-flexbox;display:flex;font-size:1.2em;font-weight:300}' +  
'.PLUGIN-program__time{-webkit-flex-shrink:0;-ms-flex-negative:0;flex-shrink:0;width:5em;position:relative}' +  
'.PLUGIN-program.focus .PLUGIN-program__time::after{content:\'\';position:absolute;top:.5em;right:.9em;width:.4em;background-color:#fff;height:.4em;-webkit-border-radius:100%;-moz-border-radius:100%;border-radius:100%;margin-top:-0.1em;font-size:1.2em}' +  
...
// Глобальні змінні для колонок  
var groupsColumn, channelsColumn, epgColumn;  
  
function epgListView(isView) {  
	// EPG тепер завжди в правій колонці  
	if (isView) {  
		epgColumn.show();  
	} else {  
		epgColumn.hide();  
...
function pluginPage(object) {  
	...
	// Створення трьохколонкової структури  
	var container = $('<div class="' + plugin.component + '__container"></div>');  
	var groupsColumn = $('<div class="' + plugin.component + '__groups"></div>');  
	var channelsColumn = $('<div class="' + plugin.component + '__channels"></div>');  
	var epgColumn = $('<div class="' + plugin.component + '__epg"></div>');  
	...
	container.append(groupsColumn).append(channelsColumn).append(epgColumn);  
	...
	layerInterval = setInterval(function() {  
		...
		if (layerMinPrev > maxI || layerMaxPrev < minI) {  
			...
			if (layerMaxPrev > maxI) layerCards.slice(layerMaxPrev, layerMaxPrev + 1).removeClass('js-layer--visible');  
	...
	this.append = function (data) {  
		...
		var bulkFn = bulkWrapper(function (channel) {  
				...
				img.onerror = function (e) {  
					...
					var hexText = (r * 0.299 + g * 0.587 + b * 0.299 + g * 0.587 + b * 0.114) > 186 ? '#000000' : '#FFFFFF';  
	...
	this.build = function (catalog) {  
		...
		channelsColumn.append(body);  
		  
		// Додавання EPG в праву колонку  
		epgColumn.empty();  
		epgColumn.append(epgTemplate);  
		...
		Lampa.Controller.add('content', {  
			...
			left: function left() {  
				if (Navigator.canmove('left')) {  
					...
					// Перехід до груп  
					Lampa.Controller.collectionSet(groupsColumn);  
					Lampa.Controller.collectionFocus($('.group-item.selected')[0] || $('.group-item')[0], groupsColumn);  
				}  
			},  
			right: function right() {  
				if (Navigator.canmove('right')) {  
					Navigator.move('right');  
				} else if (getStorage('epg', false)) {  
					// Перехід до EPG  
					Lampa.Controller.collectionSet(epgColumn);  
					Lampa.Controller.collectionFocus(epgColumn.find('.selector')[0], epgColumn);  
				} else {  
					_this.selectGroup();  
				}  
			},  
