(function () {
    ...
    // Кеш тільки для Parser / бейджів якості
    var qualityCache = {};
    var qualityLoading = {};
    var qualityTimers = {};
    ...
    function applyMovieDetailsData(data, movie, $render) {
        ...
        if (Lampa.Storage.get('tv_interface_studios')) {
            ...
            if (studio && studio.logo_path) {
                ...
                var $brand = $(
                    '<div class="studio-header-brand"><img src="' +
                    studioLogoUrl +
                    '" alt="' +
                    (studio.name || '') +
                    '"></div>'
    ...
    /**
     * ОТРИМАННЯ БЕЙДЖІВ ЯКОСТІ
     *
     * Оптимізовано:
     * 1. Результат Parser кешується.
     * 2. Повторний Parser для одного фільму не запускається.
     * 3. Parser запускається після першої фази анімації картки.
     * 4. Обробляються тільки перші 15 результатів.
     * 5. Готові бейджі кешуються окремо.
     */
    ...
    function getEliteBadges(results) {
        ...
        for (var r = 0; r < matchedBadges.length; r++) {
            ...
            if (resolutionBadge.group === 'resolution') {
                if (
                    !highestResolution ||
                    resolutionBadge.priority > highestResolution.priority
                ) {
                    highestResolution = resolutionBadge;
                }
            }
        }

        for (var m = 0; m < matchedBadges.length; m++) {
            var matched = matchedBadges[m];

            if (
                matched.group !== 'resolution' ||
                (highestResolution && matched.id === highestResolution.id)
            ) {
                foundBadges.push(matched.imageURL);
            }
        }

        if (/ukr|укр/i.test(combinedText)) {
            foundBadges.push(pluginPath + 'UKR.svg');
        }

        if (/dub|дуб/i.test(combinedText)) {
            foundBadges.push(pluginPath + 'DUB.svg');
        }
    ...
    /**
     * Отримуємо Parser тільки один раз для конкретного фільму.
     */
    function getQualityBadges(movie, callback) {
        ...
        // Уже є готовий результат
        if (qualityCache[key]) {
            callback(qualityCache[key]);
            return;
        }

        // Якщо запит уже виконується — додаємо callback у чергу
        if (qualityLoading[key]) {
            qualityLoading[key].push(callback);
            return;
        }

        qualityLoading[key] = [callback];
        ...
        Lampa.Parser.get(
            ...
            function (res) {
                ...
                if (res && Array.isArray(res.Results)) {
                    badges = getEliteBadges(res.Results);
                }

                qualityCache[key] = badges;

                var callbacks = qualityLoading[key] || [];

                delete qualityLoading[key];
    ...
    /**
     * Додає бейджі після того, як основна анімація картки
     * вже почала/завершила першу фазу.
     */
    function loadQualityBadges(movie, $qRow, startIndex) {
        if (!Lampa.Storage.get('tv_interface_quality')) {
            return;
        }
        ...
        // Скасовуємо старий таймер цього фільму
        if (qualityTimers[key]) {
            clearTimeout(qualityTimers[key]);
            delete qualityTimers[key];
        }

        // Невелика відстрочка, щоб Parser не конкурував
        // з побудовою/анімацією NewCard
        qualityTimers[key] = setTimeout(function () {
            delete qualityTimers[key];

            // Якщо результат уже в кеші — показуємо миттєво
            if (qualityCache[key]) {
                renderQualityBadges(
                    qualityCache[key],
                    $qRow,
            ...
            getQualityBadges(movie, function (eliteBadgesList) {
                // Перевіряємо, що контейнер ще належить активній картці
                if (!$qRow || !$qRow.length || !document.body.contains($qRow[0])) {
                    return;
                }
    ...
    function renderQualityBadges(badges, $qRow, startIndex) {
        if (!badges || !badges.length || !$qRow || !$qRow.length) {
            return;
        }

        // Не дублюємо бейджі
        $qRow.find('.quality-item').remove();

        var fragment = document.createDocumentFragment();

        badges.forEach(function (imgUrl, index) {
            var item = document.createElement('div');
            item.className = 'quality-item wave-item';
        ...
        Lampa.Listener.follow('full', function (e) {
            ...
            if (e.type === 'complite' || e.type === 'complete') {
                ...
                if (
                    $body.length &&
                    $buttons.length &&
                    $render.find('.card-tweaks__buttons').length === 0
                ) {
                ...
                /**
                 * ВАЖЛИВО:
                 * Тут більше немає прямого Parser.get().
                 *
                 * Він винесений у оптимізований getQualityBadges(),
                 * який має кеш і затримку запуску.
                 */
                loadQualityBadges(
                    movie,
                    $qRow,
    ...
    function setupSettings() {
        ...
        Lampa.SettingsApi.addParam({
            ...
            param: {
                ...
                values: {
                    ...
                    'breathe': 'Дихання',
                    'spin_slow': 'Гойдання',
                    'float': 'Підстрибування'
                },
                default: 'pulse'
            },
            field: {
                name: 'Анімація бейджів'
            },
            onChange: applyStyles
        });
        ...
        Lampa.SettingsApi.addParam({
            component: 'tv_interface',
            param: {
                name: 'tv_interface_show_tagline',
                type: 'trigger',
                default: true
            },
            field: {
