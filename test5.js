(function () {
    ...
    var settings_list = [
        ...
        { id: 'mobile_interface_quality_gap', default: '12px' },
    ...
    function applyStyles() {
        ...
        var qualityGap = Lampa.Storage.get('mobile_interface_quality_gap', '12px');
        ...
        // Бейджи та рейтинги тепер у загальному потоці з гнучким відступом
        css += '.quality-row-inline { ' + uiAnimClass + ' animation-delay: 0.25s; display: flex; justify-content: center; align-items: center; flex-wrap: wrap; gap: 8px 12px; width: 100%; order: 3; margin: ' + qualityGap + ' 0 !important; } '; 
        ...
        css += '.plugin-meta-row { ' + uiAnimClass + ' animation-delay: 0.20s; display: flex; justify-content: center; align-items: center; flex-wrap: nowrap; gap: 8px; margin: 0 !important; font-size: calc(' + rSize + ' * 2.5); width: 100%; order: 3; color: rgba(255,255,255,0.85); font-family: "Inter", -apple-system, system-ui, sans-serif; } ';
        
        css += '.full-start-new__tagline { ' + uiAnimClass + ' animation-delay: 0.32s; display: ' + (showTagline ? 'block' : 'none') + ' !important; font-style: italic !important; font-size: 0.9em !important; margin: 0 !important; color: rgba(255,255,255,0.8) !important; text-align: center !important; order: 5; } ';
        ...
        css += '.quality-item { transform-origin: center center; filter: drop-shadow(0 2px 5px rgba(0,0,0,0.8)); height: 1.15em; display: flex; align-items: center; justify-content: center; ';
        if (isUIAnim) {
            css += 'opacity: 0; animation: wave_cascade 0.5s cubic-bezier(0.16, 1, 0.3, 1) forwards';
            if (badgeAnim !== 'none') {
                css += ', ' + loopAnimName + ' ' + loopDuration + ' ease-in-out infinite';
    ...
    function init() {
        Lampa.Listener.follow('full', function (e) {
            ...
            if (window.innerWidth <= 480 && (e.type === 'complite' || e.type === 'complete')) {
                ...
                renderMeta($rightContainer, e);
                loadMovieDetails(movie, $render);

                $rightContainer.find('.quality-row-inline').remove();
                var $qRow = $('<div class="quality-row-inline"></div>');
                $rightContainer.append($qRow);
    ...
    function setupSettings() {
        ...
        Lampa.SettingsApi.addParam({ component: 'mobile_interface', param: { name: 'mobile_interface_quality_gap', type: 'select', values: { '6px': 'Малий', '12px': 'Стандартний', '18px': 'Великий', '24px': 'Дуже великий' }, default: '12px' }, field: { name: 'Відступ до/від бейджів якості' }, onChange: applyStyles });
