...
.${plugin.component} .card__age {  
    display: none !important;  
}  
.${plugin.component} .card__epg-progress,  
.${plugin.component} .card__epg-title {  
    display: none !important;  
}  
</style>  
`)
...
function epgRender(epgId) {  
    ...
    var card = body.find('.js-layer--visible[data-epg-id="' + epgId + '"]');  
      
    if (!card.length) return;  
      
    // Прибираємо прогрес-бар  
    card.find('.card__age').hide();  
      
    if (epgIdCurrent === epgId && epg.length) {  
        var ec = $('#' + plugin.component + '_epg');  
        var nowEl = ec.find('.js-epgNow');  
        var listEl = ec.find('.js-epgList');  
          
        // Показуємо лише поточну програму  
        if (epg[0]) {  
            nowEl.text(toLocaleTimeString(epg[0][0] * 60000) + ' - ' + epg[0][2]);  
        }  
          
        // Список наступних програм  
        listEl.empty();  
        for (var i = 1; i < Math.min(epg.length, 5); i++) {  
            var item = $('<div class="info__list-item">' +  
                toLocaleTimeString(epg[i][0] * 60000) + ' - ' + epg[i][2] +  
                '</div>');  
            listEl.append(item);  
        }  
    }  
    ...
    var card = Lampa.Template.get('card', {  
        ...
        release_year: ''  
				 ...
				 card.removeClass('layer--visible')  
