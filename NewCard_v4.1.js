    ...
    function attachLoader() {    
        Lampa.Listener.follow('full', (event) => {    
            if (event.type === 'complite') {    
                ...
                // === НОВА СИСТЕМА ФОКУСУ ===
                const buttons = render.find('.full-start-new__buttons .selector');
                const playBtn = render.find('.button--play')[0];

                Lampa.Controller.add('cas_buttons', {
                    toggle: () => {
                        Lampa.Controller.collectionSet(buttons);
                        // Завжди фокусуємося на першій кнопці (playBtn)
                        Lampa.Controller.collectionFocus(playBtn, buttons);
                    },
                    right: () => { Lampa.Select.move('right'); },
                    left: () => { Lampa.Select.move('left'); },
                    down: () => { Lampa.Controller.enable('full_descr'); }, // Перехід вниз до опису/серій
                    enter: () => { $(Lampa.Controller.enabled().target).trigger('click'); },
                    back: () => { Lampa.Activity.backward(); }
                });

                Lampa.Controller.enable('cas_buttons');
            }    
        });    
    }  
