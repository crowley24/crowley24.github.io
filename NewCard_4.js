(function () {
    ...
    // Іконка плагіна (залишається компактною)
    ...
    function initializePlugin() {
        console.log('NewCard', 'v1.0.1 - Optimized');
    ...
    function addCustomTemplate() {
        ...
        // Видалено: full-start-new__left, full-start-new__head, full-start-new__details, rating--modss, buttons--container
        ...
        <div class="full-start-new__extra-buttons hide">
    ...
    function loadNetworkIcon(activity, data) {
        ...
        if (!networkContainer.html()) {
            networkContainer.remove();
        }
    }

   function fillMetaInfo(activity, data) {
        const $render = activity.render(); // <<< Оптимізація: Кешування
    ...
    // --- Оптимізація: Кешування DOM у loadLogo ---
    function loadLogo(event) {
        ...
        const $activityRender = activity.render(); // <<< КЕШУВАННЯ
        ...
        waitForBackgroundLoad(activity, () => {
            ...
            $activityRender.find('.applecation__meta').addClass('show');      
            $activityRender.find('.applecation__info').addClass('show');       
            $activityRender.find('.applecation__ratings').addClass('show');    
        });
        ...
        $.get(apiUrl, (imagesData) => {
            // Використовуємо кешовані елементи
            const logoContainer = $activityRender.find('.applecation__logo');
            const titleElement = $activityRender.find('.full-start-new__title');
            ...
            $activityRender.find('.full-start-new__title').show();
            waitForBackgroundLoad(activity, () => {
                $activityRender.find('.applecation__logo').addClass('loaded');
            });
        });
    }
    ...
    // --- СУЧАСНИЙ CSS: backdrop-filter та затемнення 0.35 ---
    function addStyles() {
        const styles = `<style>
/* Основной контейнер */
.applecation {
...
/* Затемнення + Ефект скляного розмиття */
.full-start__background.dim {
    filter: brightness(0.35); /* Fallback: затемнення 0.35 */
}
...
.full-start__background.dim::after {
    ...
    /* Розмиття та поглиблене затемнення (основний ефект) */
...
/* Повернення до fallback (для несумісних браузерів) */
...
/* Утримуємо opacity при загрузці нового фона */
...
.applecation .full-start__status {
    ...
    var pluginManifest = {
        ...
        version: '1.0.1',
        name: 'NewCard',
        description: 'Новий дизайн картки фільму/серіалу. Оптимізовано DOM та додано ефект розмитого фону.',
        author: '',
        icon: PLUGIN_ICON
    };
