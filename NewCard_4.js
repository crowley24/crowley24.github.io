(function () {
    ...
    const translations = {
        ...
        hide_reactions: { uk: 'Приховати реакції Lampa' },
        hide_reactions_desc: { uk: 'Приховати блок з реакціями' },
        ratings_position: { uk: 'Розташування рейтингів' },
        ratings_position_desc: { uk: 'Оберіть де відображати рейтинги' },
        position_card: { uk: 'У картці' },
        position_corner: { uk: 'У лівому нижньому куті' },
        logo_size: { uk: 'Розмір логотипу' },
        logo_size_desc: { uk: 'Оберіть максимальний розмір логотипу' },
        size_small: { uk: 'Маленький' },
        size_medium: { uk: 'Середній' },
        size_large: { uk: 'Великий' }
    };
    ...
    function addSettings() {
        // Ініціалізація значень за замовчуванням, якщо не встановлені
        const defaults = {
        ...
        Lampa.SettingsApi.addParam({
            ...
            onChange: function(value) {
                ...
                addCustomTemplate();
                // Викликаємо Lampa.Activity.back() для оновлення поточної картки після зміни налаштувань.
                // Це не ідеально, оскільки повертає назад, але це найпростіший спосіб застосувати новий шаблон.
                Lampa.Activity.back(); 
            }
        });
        ...
        Lampa.SettingsApi.addParam({
            ...
            onChange: function(value) {
                $('body').toggleClass('applecation--hide-reactions', value);
            }
        });

        // Застосовуємо поточні налаштування при старті
        $('body').toggleClass('applecation--hide-ratings', !Lampa.Storage.get('applecation_show_ratings', false));
        $('body').addClass('applecation--ratings-' + Lampa.Storage.get('applecation_ratings_position', 'card'));
        $('body').toggleClass('applecation--hide-reactions', Lampa.Storage.get('applecation_hide_reactions', false));
...
/* Описание - ВИДАЛЕНО, оскільки немає у шаблоні/логіці */
...
.applecation .full-start__status {
    ...
    // Патчимо Api.img для покращеної якості фону
    ...
    function loadNetworkIcon(activity, data) {
        ...
        const entities = data.name // Серіал
            ? (data.networks || []) 
            : (data.production_companies || []); // Фільм
        
        const entity = entities.find(e => e.logo_path);

        if (entity) {
            const logoUrl = Lampa.Api.img(entity.logo_path, 'w200');
            networkContainer.html(`<img src="${logoUrl}" alt="${entity.name}">`);
        } else {
   ...
   // Заповнюємо мета інформацію (Тип/Жанр/піджанр)
    function fillMetaInfo(activity, data) {
        ...
        metaTextContainer.text(metaParts.join(' · '));
        
        // Завантажуємо іконку студії/мережі
        loadNetworkIcon(activity, data);
    }

    // Схиляння сезонів з локалізацією (лише українська логіка)
    ...
    function fillAdditionalInfo(activity, data) {
        ...
        if (data.name) { // Серіал
            ...
            if (data.runtime && data.runtime > 0) {
                ...
                let timeStr = '';
                if (hours > 0) timeStr += `${hours} ${timeH} `;
                timeStr += `${minutes} ${timeM}`;
                
                infoParts.push(timeStr.trim());
            }
        }

        infoContainer.text(infoParts.join(' · '));
    }
    ...
    function loadLogo(event) {
        ...
        // Приховуємо стандартні елементи Lampa, які нам не потрібні/дублюються
        const detailsContainer = activity.render().find('.full-start-new__details');
        if (detailsContainer.length) detailsContainer.hide().empty();
        
        const headContainer = activity.render().find('.full-start-new__head');
        if (headContainer.length) headContainer.hide().empty();
        ...
        $.get(apiUrl, (imagesData) => {
            ...
            // Обираємо найкраще лого (з логікою пріоритетів uk/en)
    ...
    function waitForBackgroundLoad(activity, callback) {
        ...
        // Функція, що завершує очікування і викликає колбек
        const finalize = () => {
        ...
        // Якщо фон завантажений, але анімація ще не завершилась
        if (background.hasClass('loaded')) {
            // Чекаємо завершення transition + невелика затримка для надійності
            setTimeout(finalize, 650); // 600ms transition + 50ms запас
            return;
        }
    ...
    var pluginManifest = {
        ...
        version: '1.0.0',
        name: 'NewCard', 
        description: 'Новий дизайн картки фільму/серіалу з акцентом на логотипі та метаінформації.',
        author: 'Community', 
        icon: PLUGIN_ICON
    };

    // Реєструємо плагін в маніфесті
