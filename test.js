(function () {
    ...
    var eliteBadgesConfig = [
        ...
        {
            id: '1080p-full-hd',
            pattern: /\b(1080p|fhd|full\s*hd)\b/i,
...

        {
            ...
            pattern: /\b(dts[\s._-]*hd[\s._-]*ma|dtshd\s*ma|dts[\s._-]*hd[\s._-]*master)\b/i,
    ...
    function applyStyles() {
         ...
         *
         * Важливо:
         * Не використовуємо негативний margin-bottom.
         * Кнопки залишаються у власній області та не обрізаються.
         */
        ...
        css += '.full-start-new .full-start__button svg, .full-start-new .full-start__button img { ' +
        ...
        /*
         * ЗМЕНШЕННЯ ВІДСТУПУ ПЕРЕД "ДЕТАЛЬНО"
         *
         * Самі кнопки не рухаємо.
         * Блок опису підтягуємо вгору окремо.
         * z-index кнопок вищий, тому опис не обрізає їх.
         */
        css += '.full-start__description, .full-start-new__description { ' +
            'position: relative !important; ' +
            'z-index: 2 !important; ' +
        ...
        css += '.full-start__description::before, .full-start-new__description::before { ' +
            'content: ""; ' +
            'position: absolute; ' +
            'left: 0; ' +
            'right: 0; ' +
    ...
    function applyMovieDetailsData(data, movie, $render) {
        ...
        if (
            ...
            var backdrops =
                data.images.backdrops.filter(function (elem) {
                    return (
                        elem.aspect_ratio > 1.5 &&
                        (!elem.iso_639_1 ||
                            elem.iso_639_1 === 'xx')
                    );
                });
    ...
    function getEliteBadges(results) {
        ...
        eliteBadgesConfig.forEach(function (badge) {
            if (
                badge.pattern &&
                badge.pattern.test(combinedText)
            ) {
        ...
        matchedBadges.forEach(function (b) {
            if (b.group === 'resolution') {
                if (
                    !highestResolution ||
                    b.priority > highestResolution.priority
                ) {
                    highestResolution = b;
                }
            }
        ...
        matchedBadges.forEach(function (badge) {
            if (
                badge.group !== 'resolution' ||
                (
                    highestResolution &&
                    badge.id === highestResolution.id
                )
            ) {
        ...
        return foundBadges.filter(function (
            elem,
            pos,
            arr
        ) {
    ...
    function startPosterSlideshow($poster, items) {
        ...
        slideshowTimer = setInterval(function () {
            ...
            if (
                !$poster ||
                $poster.length === 0 ||
                !document.body.contains($poster[0])
            ) {
                stopSlideshow();
                return;
            }
        ...
        window.mobileInterfaceBgInterval =
            slideshowTimer;
    }
    ...
    function setupSettings() {
        ...
        Lampa.SettingsApi.addParserParam =
            Lampa.SettingsApi.addParam;
