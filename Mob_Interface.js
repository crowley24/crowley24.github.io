(function () {
    ...
    function applyStyles() {
        ...
        /* ВИДАЛЯЄМО СТАТУС ТА ОЧИЩУЄМО ЗАЙВІ ВІДСТУПИ */
        css += '.full-start__status, .full-start-new__status { display: none !important; } ';
        css += '.full-start__details { display: flex !important; align-items: center !important; justify-content: center !important; flex-wrap: wrap !important; gap: 8px !important; margin: 10px 0 !important; } ';
        css += '.full-start__age { display: inline-block !important; border: 1px solid rgba(255,255,255,0.4); padding: 0 4px; border-radius: 4px; font-size: 0.9em; } ';
    ...
    function initPlugin() {
        Lampa.Listener.follow('full', function (e) {
            ...
            if (window.innerWidth <= 480 && (e.type === 'complite' || e.type === 'complete')) {
                ...
                /* ПЕРЕМІЩУЄМО ЖАНРИ ТА ТРИВАЛІСТЬ ДО ВІКОВОГО РЕЙТИНГУ */
                var $ageBlock = $render.find('.full-start__age');
                if ($ageBlock.length && $details.length) {
                    var $infoContainer = $('<div class="full-start__details"></div>');
                    $infoContainer.append($ageBlock.clone()); // копіюємо вік
                    $infoContainer.append($details.contents().clone()); // копіюємо жанри/час
                    $details.replaceWith($infoContainer); // замінюємо старий блок новим
                    $ageBlock.hide(); // ховаємо оригінальний вік в іншому місці
                }
                ...
                // Використовуємо новий контейнер для прив'язки інфоблоку
                var $newDetails = $render.find('.full-start__details').last();
