(function () {
    ...
    // =================================================================
    // 1. КОНФІГУРАЦІЯ ТА СТАН ПЛАГІНА
    // =================================================================
    var logoCache = {}; 
    var slideshowTimer; 
    ...
    function applyStyles() {
        ...
        // Видалення стандартних вікових рейтингів та інфо-панелей
        ...
        // Контейнер правої частини (інформація)
    ...
    function renderRatings(container, e) {
        ...
        // Визначаємо куди вставити (після слогана або після лого)
        var $target = container.find('.full-start-new__tagline');
        if (!$target.length || !Lampa.Storage.get('mobile_interface_show_tagline')) {
            ...
            if (source) {
                source.forEach(function(item) {
                    if (item.logo_path) {
                        ...
                        var exists = logos.some(function(l) { return l.url === url; });
                        if (!exists) logos.push({ url: url, name: item.name });
                    }
                });
            }
        });
            ...
            img.onload = function() {
                ...
                try {
                    ...
                    for(var i=0; i<d.length; i+=4) {
                        if(d[i+3]>50) { r+=d[i]; g+=d[i+1]; b+=d[i+2]; c++; }
                    }
                    if(c > 0) {
                        var brightness = (0.299*r + 0.587*g + 0.114*b) / c;
                        if(brightness < 40) $('#'+id+' img').css('filter', 'brightness(0) invert(1)');
                    }
                } catch(e) { console.log('Logo process error'); }
            };
            img.src = logo.url;
        });
    }
    ...
    function getBestResults(results) {
        ...
        results.slice(0, 15).forEach(function(item) {
            ...
            var res = null;
            if (t.indexOf('4k')>=0 || t.indexOf('2160')>=0) res = '4K';
            else if (t.indexOf('2k')>=0 || t.indexOf('1440')>=0) res = '2K';
            else if (t.indexOf('1080')>=0) res = 'FULL HD';
            else if (t.indexOf('720')>=0) res = 'HD';
            
            var order = ['HD', 'FULL HD', '2K', '4K'];
            if (res && (!best.resolution || order.indexOf(res) > order.indexOf(best.resolution))) {
                best.resolution = res;
            }
    ...
    // =================================================================
    // 4. РОБОТА З ТМDВ (ЛОГО ТА СЛАЙДШОУ)
    // =================================================================
    function loadMovieLogo(movie, $container) {
        var movieId = movie.id + (movie.name ? '_tv' : '_movie');
        var quality = Lampa.Storage.get('mobile_interface_logo_quality', 'w500');
        ...
        $.ajax({
            ...
            success: function(res) {
                ...
                if (!logo) {
                    logo = res.logos.filter(function(l) { return l.iso_639_1 === 'en'; })[0];
                }
                if (!logo && res.logos.length > 0) {
                    logo = res.logos[0];
                }
    ...
    function startPosterSlideshow($poster, items) {
        if (!Lampa.Storage.get('mobile_interface_slideshow') || items.length < 2) return;
        ...
        slideshowTimer = setInterval(function() {
            ...
            var imgUrl = Lampa.TMDB.image('/t/p/' + q + items[index].file_path);
                ...
                if (Lampa.Storage.get('mobile_interface_quality') && Lampa.Parser.get) {
                    Lampa.Parser.get({ search: movie.title || movie.name, movie: movie, page: 1 }, function(res) {
                        if (res && res.Results) {
                            ...
                            if (b.resolution) iconsToShow.push(b.resolution);
                            if (b.dolbyVision) iconsToShow.push('Dolby Vision');
                            else if (b.hdr) iconsToShow.push('HDR');
                            if (b.dub) iconsToShow.push('DUB');
                            if (b.ukr) iconsToShow.push('UKR');
                            
                            iconsToShow.forEach(function(iconName, i) {
                                if (svgIcons[iconName]) {
                                    var $item = $('<div class="quality-item" style="animation-delay:'+(i*0.1)+'s"><img src="'+svgIcons[iconName]+'"></div>');
                                    $infoBlock.find('.quality-row').append($item);
                                }
                            });
                        }
                    });
    ...
    function setupSettings() {
        ...
        Lampa.SettingsApi.addParam({
            component: 'mobile_interface',
            param: { name: 'mobile_interface_blocks_gap', type: 'select', values: { '2px': 'Дуже щільно', '5px': 'Малі', '8px': 'Середні', '12px': 'Великі', '18px': 'Просторі' }, default: '8px' },
        ...
        Lampa.SettingsApi.addParam({
            component: 'mobile_interface',
            param: { name: 'mobile_interface_logo_size_v2', type: 'select', values: { '60': '60px', '80': '80px', '100': '100px', '120': '120px' }, default: '90' },
    ...
    function startPlugin() {
        ...
        // Вимкнення стандартного розмиття
