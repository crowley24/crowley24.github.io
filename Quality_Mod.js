(function () {
    ...
    // =======================================================
    // I. КОНФІГУРАЦІЯ (Покращення: Централізовано)
    // =======================================================
    var Config = {
        // Загальні
        Q_LOGGING: true,
        ...
        // JACRED_API_KEY: Lampa.Storage.get(''), // Залишаємо як було, якщо використовується
    ...
    var Utils = {
        ...
        /**
         * Моніторинг продуктивності (залишено як було)
         */
        performance: {
            timers: {},
        ...
        /**
         * Статистика виконання (залишено як було)
         */
        stats: {
            requests: 0,
            cacheHits: 0,
    ...
    // =======================================================
    // IV. МОДУЛЬ КАШУВАННЯ (Покращення: N/A - вже добре з try/catch)
    // =======================================================
    var Cache = {
        /**
        ...
        get: function(key) {
            try {
                // Використовуємо Config.QUALITY_CACHE
                ...
                // Обробка помилок localStorage (наприклад, переповнення)
    ...
    // =======================================================
    var QualityDetector = {
        // Логіка QualityDetector залишається, оскільки вона незалежна
        ...
        getBestQuality: function(qualities) {
            ...
            return qualities.reduce(function(best, current) {
                ...
                // Оскільки масив пріоритету від вищого до нижчого, індекс менший для кращої якості
    ...
    // =======================================================
    // VI. МОДУЛЬ UI (Покращення: N/A - вже добре)
    // =======================================================
    var UI = {
        /**
        ...
        initStyles: function() {
            ...
            // CSS залишається без змін, оскільки він оптимізований
        ...
        /**
         * Додавання бейджа якості з санітизацією (DOM-залежність залишається)
         */
        addQualityBadge: function(card, quality) {
            // Перевірка наявності елемента перед маніпуляцією DOM
    ...
    * Функція отримання якості з JacRed з покращеною обробкою
    ...
    function getBestReleaseFromJacred(normalizedCard, cardId, callback) {
        ...
        function searchJacredApi(searchTitle, searchYear, exactMatch, strategyName, apiCallback) {
            ...
            if (Config.Q_LOGGING) Utils.logWithContext('log', 'JacRed: ' + strategyName + ' URL', { url: apiUrl, cardId: cardId });
            ...
            API.queueRequest(apiUrl, cardId, function(error, responseText) { // Використовуємо queueRequest
                ...
                if (!responseText) {
                    if (Config.Q_LOGGING) Utils.logWithContext('log', 'JacRed: ' + strategyName + ' failed or empty response', { cardId: cardId });
                    apiCallback(null);
                    return;
                }
                ...
                try {
                    ...
                    if (!Array.isArray(torrents) || torrents.length === 0) {
                        ...
                        if (exactMatch) {
                            ...
                            // Рекурсивний виклик з менш строгим пошуком, але зберігаємо стратегію для логування
                    ...
                    // Логіка оцінки (score) торрентів (залишається, оскільки API не дає "найкращий реліз")
                    var scoredTorrents = torrents.map(function(torrent) {
                        ...
                        // Бали за розмір файлу (макс. 5 балів)
                        if (torrent.size) {
                            score += Math.min(torrent.size / (1024 * 1024 * 1024), 5);
                        }

                        // Мінус бали за низьку якість (TS/CAM)
        ...
        function executeNextStrategy(index) {
            ...
            searchJacredApi(strategy.title, strategy.year, strategy.exact, strategy.name, function(result) {
                if (result !== null && result.quality !== null) { // Перевірка, що якість не null
                    if (Config.Q_LOGGING) Utils.logWithContext('log', 'Successfully found quality', { quality: result.quality, cardId: cardId });
                    callback(result);
                } else {
    ...
    function updateCards(cards) {
        ...
        // Очищення кешу при старті (залишаємо, але вона викликається рідко)
        Cache.cleanup();
        ...
        for (var i = 0; i < cards.length; i += Config.BATCH_SIZE) {
            ...
            batch.forEach(function(card) {
                // Перевірка DOM-залежностей: наявність атрибуту data-quality-added
                ...
                (function (currentCard) {
                    ...
                    if (cacheQualityData) {
                        if (Config.Q_LOGGING) Utils.logWithContext('log', 'Using cached quality', { cardId: localCurrentCard, quality: cacheQualityData.quality });
        ...
        for (var m = 0; m < mutations.length; m++) {
            ...
            if (mutation.addedNodes) {
                for (var j = 0; j < mutation.addedNodes.length; j++) {
                    ...
                    // Зменшення DOM-залежності шляхом універсального пошуку
    ...
    function startPlugin() {
        console.log("MAXSM-RATINGS-QUALITY", "Plugin started with structural improvements!");
        ...
        if (localStorage.getItem('maxsm_ratings_quality_inlist') === 'true') {
            // Observer.observe має бути спрямований на найвищий елемент, який часто змінюється
