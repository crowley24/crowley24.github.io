...
var icon_add_radio_plugin = '<div class="settings-folder" style="padding:0!important"><div style="width:1.8em;height:1.3em;padding-right:.5em"><svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 26 26"><path fill="currentColor" d="M23.5.063c-.794 0-1.438.643-1.438 1.437L4.657 6.313c-.2-.076-.43-.125-.656-.125A1.81 1.81 0 0 0 0 1.81 1.81 0 0 0 2.187 8v.125C.933 8.484 0 9.63 0 11v11c0 1.656 1.344 3 3 3h20c1.656 0 3-1.344 3-3V11c0-1.656-1.344-3-3-3H3c-1.656 0-3 1.344-3 3v11c0 1.656 1.344 3 3 3m20.5-21.937c.794 0 1.438.643 1.438 1.437s-.644 1.438-1.438 1.438c-.793 0-1.437-.644-1.437-1.438s.644-1.437 1.437-1.437M6.5 6.188c.273 0 .503.049.703.125L22.062 1.5c0-.794.644-1.437 1.438-1.437c.793 0 1.437.643 1.437 1.437c0 .4-.163.76-.425 1.019l-16.656 5c-.25.428-.688.719-1.219.719a1.437 1.437 0 1 1 0-2.876m-6 10.75a5.696 5.696 0 0 1 5.688 5.687a5.696 5.696 0 0 1-5.688 5.688a5.697 5.697 0 0 1-5.688-5.688a5.697 5.697 0 0 1 5.688-5.687m-13 .093c.877 0 1.594.717 1.594 1.594s-.717 1.594-1.594 1.594A1.597 1.597 0 0 1 2.906 12.5c0-.877.716-1.594 1.594-1.594m13 1.281c-.937 0-1.793.306-2.5.813h5a4.26 4.26 0 0 0-2.5-.813M14 14a4.3 4.3 0 0 0-.531 1h8.062A4.4 4.4 0 0 0 21 14zm-9.5 1.906c.877 0 1.594.717 1.594 1.594s-.717 1.594-1.594 1.594A1.597 1.597 0 0 1 2.906 17.5c0-.877.716-1.594 1.594-1.594m8.75.094c-.02.166-.063.328-.063.5s.043.334.063.5h8.5c.02-.166.063-.328.063-.5s-.044-.334-.063-.5zm.219 2q.202.538.531 1h7q.33-.462.531-1H13.47zM15 20a4.27 4.27 0 0 0 2.5.813c.938 0 1.793-.306 2.5-.813z"/></svg></div><div style="font-size:1.3em">Радио</div></div>'  
...
function itemON(sourceURL, sourceName, sourceAuthor, itemName) {  
    ...
    // Проверяем, существует ли уже такой плагин  
    var existingPlugin = plugins.find(function(plugin) {  
        return plugin.url === sourceURL;  
    });  
      
    if (existingPlugin) {  
        // Если плагин существует, обновляем его статус  
        existingPlugin.status = 1;  
    } else {  
        // Если плагин не существует, добавляем его  
        plugins.push(newPlugin);  
    }  
      
    // Сохраняем обновленный массив плагинов  
