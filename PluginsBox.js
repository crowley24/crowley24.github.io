...
function addonStart() {  
    ...
    // Themes - Додано напряму в add_plugin  
    Lampa.SettingsApi.addParam({  
        component: 'add_plugin',  
        param: {  
            name: 'THEMES',  
        ...
        field: {  
            name: 'Теми для інтерфейса',  
            description: 'Теми для інтерфейсу'  
        },  
        onChange: function(value) {  
            if (value == '1') {  
                itemON('https://crowley24.github.io/themes.js', 'Themes', '@lampa', 'THEMES');  
            }  
            if (value == '2') {  
                deletePlugin("https://crowley24.github.io/themes.js");  
            }  
        },  
        onRender: function(item) {  
            $('.settings-param__name', item).css('color', '#f3d900');  
            ...
            var myResult = checkPlugin('https://crowley24.github.io/themes.js');  
            var pluginsArray = Lampa.Storage.get('plugins') || [];  
              
            setTimeout(function() {  
                var container = $('div[data-name="THEMES"]');  
                if (container && container.length && container.find('.settings-param__status').length === 0) {  
                    container.append('<div class="settings-param__status one"></div>');  
                }  
                var pluginStatus = null;  
                for (var i = 0; i < pluginsArray.length; i++) {  
                    if (pluginsArray[i] && pluginsArray[i].url === 'https://crowley24.github.io/themes.js') {  
                        pluginStatus = pluginsArray[i].status;  
                        break;  
                    }  
                }  
                var statusEl = $('div[data-name="THEMES"]').find('.settings-param__status');  
                if (statusEl && statusEl.length) {  
                    statusEl.removeClass('active error').css('background-color', '');  
                    if (myResult && pluginStatus !== 0) {  
                        statusEl.addClass('active');  
