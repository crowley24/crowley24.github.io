(function () {
    ...
    function initializePlugin() {
        // ВИДАЛЕНО: Lampa.Platform.screen('tv') - тепер працює всюди
        addCustomTemplate();
        addStyles();
        addSettings();
        attachLogoLoader();
    ...
    function applyScales() {
        ...
        root.style.setProperty('--apple-logo-scale', (parseInt(Lampa.Storage.get('applecation_logo_scale')) || 100) / 100);
        root.style.setProperty('--apple-text-scale', (parseInt(Lampa.Storage.get('applecation_text_scale')) || 100) / 100);
    }
    ...
    function addStyles() {
        ...
        <style>
            ...
            .applecation__studios img { max-height: 24px; margin-right: 8px; vertical-align: middle; }
            ...
            .applecation__buttons-row { display: flex; align-items: center; gap: 20px; flex-wrap: wrap; }
        ...
        if (!$('style#apple-styles').length) $('body').append('<style id="apple-styles">' + styles + '</style>');
    }
    ...
    function loadLogo(event) {
        ...
        // ВИПРАВЛЕНО: Використання Lampa.Network.native для обходу CORS в браузері
        const url = Lampa.TMDB.api(`${data.name ? 'tv' : 'movie'}/${data.id}/images?api_key=${Lampa.TMDB.key()}`);
        
        Lampa.Network.native(url, (d) => {
            if (d && d.logos && d.logos.length) {
