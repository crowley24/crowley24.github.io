                    ...
                    if (logoPath !== '') {
                        ...
                        if (window.innerWidth > 585) {
                            if (Lampa.Storage.get('card_interfice_type') === 'new' && !card.find('div[data-name="card_interfice_cover"]').length) {
                                ...
                                logoHtml = '<div style="display: inline-block; height: 2.8em; width: auto; max-width: 100%;"><img style="height: 100%; width: auto; object-fit: contain; display: block; margin-bottom: 0.2em;" src="' + Lampa.TMDB.image('/t/p/w500' + logoPath.replace('.svg', '.png')) + '" /></div>' + (titleText ? '<span style="display: block;">' + titleText + '</span>' : '');
                                card.find('.full-start-new__title').html(logoHtml);
                            } else if (Lampa.Storage.get('card_interfice_type') === 'old' && !card.find('div[data-name="card_interfice_cover"]').length) {
                                logoHtml = '<div style="height: auto !important; overflow: visible !important;"><div style="display: inline-block; height: 80px; width: auto; max-width: 100%;"><img style="height: 100%; width: auto; object-fit: contain; display: block; margin-bottom: 0em;" src="' + Lampa.TMDB.image('/t/p/w300' + logoPath.replace('.svg', '.png')) + '" /></div>' + (titleText ? '<span style="display: block; line-height: normal;">' + titleText + '</span>' : '') + '</div>';
                            ...
                            if (Lampa.Storage.get('card_interfice_type') === 'new') {
                                logoHtml = '<div style="display: inline-block; height: 1.8em; width: auto; max-width: 100%;"><img style="height: 100%; width: auto; object-fit: contain; display: block; margin-bottom: 0.2em;" src="' + Lampa.TMDB.image('/t/p/w500' + logoPath.replace('.svg', '.png')) + '" /></div>' + (titleText ? '<span style="display: block;">' + titleText + '</span>' : '');
                                card.find('.full-start-new__tagline').remove();
                                card.find('.full-start-new__title').html(logoHtml);
                            } else {
                                logoHtml = '<div style="height: auto !important; overflow: visible !important;"><div style="display: inline-block; height: 38px; width: auto; max-width: 100%;"><img style="height: 100%; width: auto; object-fit: contain; display: block; margin-bottom: 0em;" src="' + Lampa.TMDB.image('/t/p/w300' + logoPath.replace('.svg', '.png')) + '" /></div>' + (titleText ? '<span style="display: block; line-height: normal;">' + titleText + '</span>' : '') + '</div>';
                                card.find('.full-start__title-original').remove();
                                card.find('.full-start__title').css({
                                    'height': 'auto !important',
                                    'max-height': 'none !important',
