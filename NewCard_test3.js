(function () {  
    ...
    // Нові константи з другого плагіна  
    ...
    // Нова функція отримання кольору рейтингу  
    function getRatingColor(val) {  
        const n = parseFloat(val);  
        return n >= 7.5 ? '#2ecc71' : n >= 6 ? '#feca57' : '#ff4d4d';  
    }  
  
    // Нова функція форматування часу  
    ...
    // Функції кешування з другого плагіна  
    ...
    function loadLogo(event) {  
        ...
        const qualityContainer = render.find('.applecation__quality');  
        const descriptionContainer = render.find('.applecation__description');  
        ...
        fillQuality(qualityContainer, data);  
        fillDescription(descriptionContainer, data);  
        ...
        $.get(apiUrl, (imagesData) => {  
            ...
            if (bestLogo) {  
                const logoQuality = Lampa.Storage.get('applecation_logo_quality', 'original');  
                const logoPath = bestLogo.file_path;  
                const logoUrl = Lampa.TMDB.image(`/t/p/${logoQuality}${logoPath}`);  
    ...
    const translations = {  
        ...
        show_ratings_desc: { uk: 'Відображати рейтинги TMDB та CUB' },  
        show_quality: { uk: 'Показувати якість' },  
        show_quality_desc: { uk: 'Відображати іконки якості (4K, HD, HDR тощо)' },  
        show_description: { uk: 'Опис фільму' },  
        show_description_desc: { uk: 'Показувати опис фільму' },  
        logo_quality: { uk: 'Якість логотипу' },  
        logo_quality_desc: { uk: 'Якість завантаження логотипу фільму' },  
    ...
    function addSettings() {  
        ...
        const defaults = {  
            ...
            'applecation_show_description': true,  
            'applecation_logo_scale': '100',  
            'applecation_logo_quality': 'original',  
        ...
        // Нова функція - показувати якість  
        Lampa.SettingsApi.addParam({  
            component: 'applecation_settings',  
            param: { name: 'applecation_show_quality', type: 'trigger', default: true },  
            field: { name: t('show_quality'), description: t('show_quality_desc') },  
            onChange: (value) => Lampa.Storage.set('applecation_show_quality', value)  
        });  
  
        // Нова функція - опис фільму  
        Lampa.SettingsApi.addParam({  
            component: 'applecation_settings',  
            param: { name: 'applecation_show_description', type: 'trigger', default: true },  
            field: { name: t('show_description'), description: t('show_description_desc') },  
            onChange: (value) => Lampa.Storage.set('applecation_show_description', value)  
        });  
  
        // Показувати рейтинги (оновлено)  
        Lampa.SettingsApi.addParam({  
            component: 'applecation_settings',  
...
.applecation__rate-items {  
    display: flex;  
    align-items: center;  
    gap: 12px;  
}  
  
.applecation__rate-item {  
    display: flex;  
    align-items: center;  
    gap: 6px;  
}  
  
.applecation__rate-item img {  
    height: 1.2em;  
    width: auto;  
}  
  
.applecation__rate-item span {  
    font-weight: 600;  
    font-size: 0.95em;  
}  
...
body.applecation--hide-ratings .applecation__ratings {  
    ...
    function registerPlugin() {  
        const pluginManifest = {  
            ...
            version: '1.2.0',  
            name: 'NewCard',  
            description: 'Новий дизайн картки фільму/серіалу з розширеними функціями.',  
            author: '',  
            icon: PLUGIN_ICON  
        };  
