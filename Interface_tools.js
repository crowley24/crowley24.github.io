(function() {  
    ...
    function loadScript(url, callback) {  
        try {  
            ...
            console.error('Помилка при створенні скрипту:', e);  
        }  
    }  
    ...
    function init() {  
        try {  
            // Перевіряємо доступність Lampa  
            if (!window.Lampa) {  
                console.error('Lampa не знайдено');  
                return;  
            }  
  
            // Перевіряємо доступність jQuery  
            if (!window.$) {  
                console.error('jQuery не знайдено');  
                return;  
            }  
            ...
            Lampa.Settings.listener.follow('open', function(e) {  
                try {  
                    ...
                    console.error('Помилка в обробнику налаштувань:', e);  
                }  
            });  
            ...
            function updateSettingsDisplay() {  
                try {  
                    ...
                    console.error('Помилка в updateSettingsDisplay:', e);  
                }  
            }  
            ...
            console.error('Помилка ініціалізації плагіну:', e);  
        }  
    }  
  
    // Запуск плагіну з перевірками  
    if (window.Lampa && window.$) {  
        ...
        setTimeout(function() {  
            clearInterval(checkInterval);  
            if (!window.Lampa) {  
                console.error('Lampa не завантажився протягом 10 секунд');  
            }  
        }, 10000);  
    }  
