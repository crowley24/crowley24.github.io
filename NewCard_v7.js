(function () {                  
    ...
    function applySettings() {            
        ...
        if (currentCard.length > 0) {            
            ...
            currentCard.find('.cas-rating-corner').toggle(!!Lampa.Storage.get('cas_show_rating'));  
    ...
    function addCustomTemplate() {                         
        ...
        const template = `<div class="full-start-new left-title cas-anim-${animStyle} cas-loading-state">                        
            <div class="full-start-new__body">                        
                ...
                <div class="cas-rating-corner"></div>                    
            </div>                    
...
function addStyles() {    
    ...
    .cas-logo, .cas-studios-row, .cas-rate-items, .cas-meta-info, .cas-quality-row, .cas-description, .cas-details-wrapper, .full-start-new__buttons, .cas-rating-corner {    
        opacity: 0 !important;    
    ...
    .cas-anim-slide .cas-rating-corner {    
    ...
    .cas-anim-spring .cas-rating-corner {    
    ...
    .cas-animated .cas-rating-corner {  
...
function cleanup() {  
    ...
    card.removeClass('cas-animated cas-loading-state');  
      
    // Скидаємо стилі анімації для всіх елементів  
    card.find('.cas-logo, .cas-studios-row, .cas-rate-items, .cas-meta-info, .cas-quality-row, .cas-description, .cas-details-wrapper, .full-start-new__buttons, .cas-rating-corner').each(function() {  
        this.style.transition = 'none';  
        this.style.transform = '';  
        this.style.opacity = '';  
    });  
      
    // Примусово перерендеримо для скидання transitions  
...
function attachLoader() {  
    Lampa.Listener.follow('full', (event) => {  
        if (event.type === 'complite') {  
            ...
            // Примусово скидаємо transitions перед новою анімацією  
            cardRoot.find('.cas-logo, .cas-studios-row, .cas-rate-items, .cas-meta-info, .cas-quality-row, .cas-description, .cas-details-wrapper, .full-start-new__buttons, .cas-rating-corner').each(function() {  
                this.style.transition = 'none';  
                this.style.transform = '';  
                this.style.opacity = '';  
            });  
            ...
            if (data && data.id) {  
                ...
                const isTV = /tv|webos|tizen|smarttv/i.test(navigator.userAgent);  
                const tvDelay = isTV ? 300 : 0;  
