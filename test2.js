(function () {
    ...
    var settings_list = [
        ...
        { id: 'mobile_interface_tv_mode', default: true }
    ];
    ...
    function applyStyles() {
        ...
        var isTvMode = Lampa.Storage.get('mobile_interface_tv_mode', true);
        ...
        // Застосовуємо загальні стилі інтерфейсу без жорсткого обмеження екрана (працює і на Smart TV, і на телефоні)
        ...
        css += '.full-start-new__poster { position: relative !important; overflow: hidden !important; background: #000; z-index: 1; height: ' + (isTvMode ? '50vh' : '62vh') + ' !important; ';
        ...
        css += '.full-start-new__right { background: none !important; margin-top: ' + (isTvMode ? '-120px' : '-160px') + ' !important; z-index: 2 !important; display: flex !important; flex-direction: column !important; align-items: center !important; padding: 0 15px !important; gap: ' + blocksGap + ' !important; position: relative !important; } ';
        ...
        css += '.quality-row-inline { position: absolute; top: 30px; right: 20px; z-index: 99; display: flex; flex-direction: column; align-items: flex-end; gap: 6px; pointer-events: none; } '; 

        css += '.studio-header-brand { ' + uiAnimClass + ' animation-delay: 0.08s; order: 1; width: 100%; display: flex; justify-content: flex-start; align-items: center; padding-left: 3vw; margin-bottom: -2px !important; } ';
        css += '.studio-header-brand img { height: ' + (isTvMode ? '24px' : '18px') + ' !important; width: auto; max-width: 140px; object-fit: contain; filter: drop-shadow(0 2px 5px rgba(0,0,0,0.9)); opacity: 0.95; } ';
        ...
        css += '.full-start-new__title img { height: auto !important; max-height: ' + lHeight + 'px !important; width: auto !important; max-width: 85vw !important; object-fit: contain !important; filter: drop-shadow(0 4px 20px rgba(0,0,0,0.9)); margin: 0 !important; } ';

        css += '.full-start-new__tagline { ' + uiAnimClass + ' animation-delay: 0.22s; display: ' + (showTagline ? 'block' : 'none') + ' !important; font-style: italic !important; font-size: ' + (isTvMode ? '1.1em' : '0.9em') + ' !important; margin: 0 !important; color: rgba(255,255,255,0.8) !important; text-align: center !important; order: 3; } ';
        
        css += '.plugin-meta-row { ' + uiAnimClass + ' animation-delay: 0.28s; display: flex; justify-content: center; align-items: center; flex-wrap: nowrap; gap: 10px; margin: 0 !important; font-size: calc(' + rSize + ' * 2.2); width: 100%; order: 4; color: rgba(255,255,255,0.85); font-family: "Inter", -apple-system, system-ui, sans-serif; } ';
        ...
        css += '.quality-row-inline .plugin-rating-item { display: flex; align-items: center; gap: 6px; font-weight: 700; color: #fff; font-size: ' + (isTvMode ? '1.2em' : '0.95em') + '; padding: 2px 0; } ';
        css += '.quality-row-inline .plugin-rating-item img { height: 1.2em; width: auto; } ';
        
        css += '.quality-item { transform-origin: center center; filter: drop-shadow(0 2px 5px rgba(0,0,0,0.8)); height: ' + (isTvMode ? '1.5em' : '1.15em') + '; display: flex; align-items: center; justify-content: flex-end; ';
        if (isUIAnim) {
            css += 'opacity: 0; animation: wave_cascade 0.5s cubic-bezier(0.16, 1, 0.3, 1) forwards';
            if (badgeAnim !== 'none') {
                css += ', ' + loopAnimName + ' ' + loopDuration + ' ease-in-out infinite';
        ...
        css += '} ';
        css += '.quality-item img { height: 100%; width: auto; max-width: 80px; object-fit: contain; } ';

        css += '.info-text-item { opacity: 0.9; font-weight: 500; font-size: ' + (isTvMode ? '1em' : '0.85em') + '; white-space: nowrap; } ';
        css += '.info-separator { opacity: 0.35; font-size: 0.8em; margin: 0 4px; } ';

        css += '.full-start-new__buttons { ' + uiAnimClass + ' animation-delay: 0.42s; display: flex !important; justify-content: center !important; gap: 20px !important; align-items: center !important; width: 100% !important; max-width: 100% !important; padding: 0 10px !important; box-sizing: border-box !important; margin-top: 6px !important; order: 6; } ';
        css += '.full-start-new .full-start__button { background: none !important; border: none !important; box-shadow: none !important; display: flex !important; flex-direction: column !important; align-items: center !important; width: auto !important; min-width: 50px !important; padding: 0 !important; transition: transform 0.2s ease, opacity 0.2s ease; } ';
        css += '.full-start-new .full-start__button:active, .full-start-new .full-start__button:focus { transform: scale(1.1); opacity: 1; } ';
        css += '.full-start-new .full-start__button svg, .full-start-new .full-start__button img { width: ' + (isTvMode ? '32px' : '24px') + ' !important; height: ' + (isTvMode ? '32px' : '24px') + ' !important; margin-bottom: 4px !important; fill: #fff !important; filter: drop-shadow(0 2px 8px rgba(0,0,0,0.5)); } ';
        css += '.full-start-new .full-start__button span { font-size: ' + (isTvMode ? '11px' : '9px') + ' !important; text-transform: uppercase !important; opacity: 0.75 !important; font-weight: 600; letter-spacing: 0.05em; } ';
    ...
    function setupSettings() {
        ...
        Lampa.SettingsApi.addParam({ component: 'mobile_interface', param: { name: 'mobile_interface_logo_size_v2', type: 'select', values: { '125': 'Малий', '150': 'Середній', '180': 'Стандартний', '210': 'Великий' }, default: '150' }, field: { name: 'Висота логотипу тайтлу' }, onChange: applyStyles });
        ...
        Lampa.SettingsApi.addParam({ component: 'mobile_interface', param: { name: 'mobile_interface_blocks_gap', type: 'select', values: { '8px': 'Компактний', '12px': 'Стандартний', '18px': 'Просторий', '24px': 'Панорамний' }, default: '12px' }, field: { name: 'Відступи між блоками' }, onChange: applyStyles });
        Lampa.SettingsApi.addParserParam = Lampa.SettingsApi.addParam; 
        Lampa.SettingsApi.addParam({ component: 'mobile_interface', param: { name: 'mobile_interface_ratings_size', type: 'select', values: { '0.4em': 'Дрібний', '0.45em': 'Звичайний', '0.5em': 'Великий', '0.55em': 'Дуже великий' }, default: '0.55em' }, field: { name: 'Розмір шрифту інфо-блоків' }, onChange: applyStyles });
        ...
        Lampa.SettingsApi.addParam({ component: 'mobile_interface', param: { name: 'mobile_interface_tv_mode', type: 'trigger', default: true }, field: { name: 'Оптимізація під Smart TV (збільшений розмір)' }, onChange: applyStyles });
    }
