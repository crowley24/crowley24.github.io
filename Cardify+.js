(function () {  
    ...
    const translations = {  
        ...
        // Нові переклади для перемикання постерів  
        show_poster_switcher: {  
            uk: 'Перемикання постерів'  
        },  
        show_poster_switcher_desc: {  
            uk: 'Показувати кнопки для перемикання альтернативних постерів'  
        },  
        poster_switcher_position: {  
            uk: 'Розташування кнопок'  
        },  
        poster_switcher_position_desc: {  
            uk: 'Де показувати кнопки перемикання постерів'  
        },  
        position_poster: {  
            uk: 'На постері'  
        },  
    ...
    function addSettings() {  
        ...
        // Нові налаштування для перемикання постерів  
        if (Lampa.Storage.get('applecation_show_poster_switcher') === undefined) Lampa.Storage.set('applecation_show_poster_switcher', true);  
        if (Lampa.Storage.get('applecation_poster_switcher_position') === undefined) Lampa.Storage.set('applecation_poster_switcher_position', 'poster');  
        ...
        // 8. Перемикання постерів - нова функція  
        Lampa.SettingsApi.addParam({  
            component: 'applecation_settings',  
            param: {  
                name: 'applecation_show_poster_switcher',  
                type: 'trigger',  
                default: true  
            },  
            field: {  
                name: t('show_poster_switcher'),  
                description: t('show_poster_switcher_desc')  
            },  
            onChange: function(value) {  
                Lampa.Storage.set('applecation_show_poster_switcher', value);  
                $('body').toggleClass('applecation--hide-poster-switcher', !value);  
            }  
        });  
  
        // 9. Розташування кнопок перемикання постерів  
        Lampa.SettingsApi.addParam({  
            component: 'applecation_settings',  
            param: {  
                name: 'applecation_poster_switcher_position',  
                type: 'select',  
                values: {  
                    poster: t('position_poster'),  
                    background: t('position_background')  
                },  
                default: 'poster'  
            },  
            field: {  
                name: t('poster_switcher_position'),  
                description: t('poster_switcher_position_desc')  
            },  
            onChange: function(value) {  
                Lampa.Storage.set('applecation_poster_switcher_position', value);  
                $('body').removeClass('applecation--poster-switcher-poster applecation--poster-switcher-background');  
                $('body').addClass('applecation--poster-switcher-' + value);  
            }  
        });  
        ...
        if (!Lampa.Storage.get('applecation_show_poster_switcher', true)) {  
            $('body').addClass('applecation--hide-poster-switcher');  
        }  
        $('body').addClass('applecation--ratings-' + Lampa.Storage.get('applecation_ratings_position', 'card'));  
        $('body').addClass('applecation--poster-switcher-' + Lampa.Storage.get('applecation_poster_switcher_position', 'poster'));  
        applyScales();  
    }  
        ...
        <div class="full-start-new__body">  
            <div class="full-start-new__left hide">  
                <div class="full-start-new__poster">  
                    ...
                    <!-- Новий блок для перемикання постерів -->  
                    <div class="applecation__poster-switcher" style="display: none;">  
                        <div class="poster-switcher__prev selector">  
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none">  
                                <path d="M15 18L9 12L15 6" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>  
                            </svg>  
                        </div>  
                        <div class="poster-switcher__counter">  
                            <span class="current">1</span> / <span class="total">1</span>  
                        </div>  
                        <div class="poster-switcher__next selector">  
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none">  
        ...
        <div class="hide buttons--container">  
            <div class="full-start__button view--torrent hide">  
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 50 50" width="50px" height="50px">  
                    <path d="M25,2C12.317,2,2,12.317,2,25s10.317,23,23,23s23-10.317,23-23S37.683,2,25,2z M40.5,30.963c-3.1,0-4.9-2.4-4.9-2.4 S34.1,35,27,35c-1.4,0-3.6-0.837-3.6-0.837c-1.4,0-3.6-0.837-3.6-0.837L9.237,16.851 c0,0-0.7-1.2,0.4-1.5c1.1-0.3,5.4-1.2,5.4-1.2s1.475-0.494,1.8,0.5c0.5,1.3,4.063,11.112,4.063,11.112S22.6,29,27.4,29 c4.7,0,5.9-3.437,5.7-3.937c-1.2-3-4.993-11.862-4.993-11.862s-0.6-1.1,0.8-1.4c1.4-0.3,3.8-0.7,3.8-0.7s1.105-0.163,1.6,0.8 c0.738,1.437,5.193,11.262,5.193,11.262s1.1,2.9,3.3,2.9c0.464,0,0.834-0.046,1.152-0.104c-0.082,1.635-0.348,3.221-0.817,4.722 C42.541,30.867,41.756,30.963,40.5,30.963z" fill="currentColor"/>  
                </svg>  
...
/* Перемикання постерів - нові стилі */  
.applecation__poster-switcher {  
    position: absolute;  
    bottom: 1em;  
    left: 50%;  
...
.applecation__poster-switcher.show {  
    opacity: 1;  
}  
  
.poster-switcher__prev,  
.poster-switcher__next {  
    display: flex;  
    align-items: center;  
...
.poster-switcher__prev:hover,  
.poster-switcher__next:hover {  
    background: rgba(255, 255, 255, 0.2);  
}  
  
.poster-switcher__counter {  
    color: #fff;  
    font-size: 0.9em;  
    font-weight: 500;  
    min-width: 3em;  
...
/* Розташування перемикача на фоні */  
body.applecation--poster-switcher-background .applecation__poster-switcher {  
    bottom: 1em;  
    left: 1em;  
    transform: none;  
}  
  
/* Приховування перемикача */  
body.applecation--hide-poster-switcher .applecation__poster-switcher {  
...
// Функція завантаження перемикача постерів  
function loadPosterSwitcher(activity, data) {  
    const showSwitcher = Lampa.Storage.get('applecation_show_poster_switcher', true);  
    ...
    $.get(apiUrl, (imagesData) => {  
        ...
        function changePoster(index) {  
            ...
            const posterUrl = Lampa.TMDB.image(`/t/p/w500${poster.file_path}`);  
...
// Заповнюємо додаткову інформацію  
function fillAdditionalInfo(activity, data) {  
    const infoContainer = activity.render().find('.applecation__info');  
    const infoParts = [];  
...
// Завантажуємо логотип та постери  
function loadLogo(event) {  
    const data = event.data.movie;  
    const activity = event.object.activity;  
    ...
    $.get(apiUrl, (imagesData) => {  
        ...
        loadPosterSwitcher(activity, data);  
...
var pluginManifest = {  
    ...
    version: '1.1.0',  
    name: 'NewCard',  
    description: 'Новий дизайн картки фільму/серіалу з перемиканням постерів.',  
    author: '',  
    icon: PLUGIN_ICON  
};  
