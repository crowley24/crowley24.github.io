(function () {
  ...
  function getSvgIcon(label, isDolby) {
    ...
    if (isDolby) {
      content = 
        ...
        '<g transform="translate(12, 27) scale(1.05)">' +
        ...
        // Вертикальний розділювач (по центру між лого і текстом)
        '<line x1="44" y1="12" x2="44" y2="42" stroke="' + col.stroke + '" stroke-width="1.2" opacity="0.5"/>' +
  ...
  var style = '<style>\
    ...
    .quality-badge { height: 2.1em; opacity: 0; transform: scale(0.8); animation: qb_pop 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275) forwards; }\
    .card-quality-badges { position: absolute; top: 0.4em; right: 0.4em; display: flex; flex-direction: row; gap: 0.25em; z-index: 5; }\
    .card-quality-badge { height: 1.35em; opacity: 0; transform: scale(0.8); animation: qb_pop 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275) forwards; }\
    @keyframes qb_pop { to { opacity: 1; transform: scale(1); } }\
    .quality-badge img, .card-quality-badge img { height: 100%; width: auto; display: block; filter: drop-shadow(0 2px 6px rgba(0,0,0,0.7)); }\
