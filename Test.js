(function () {
    ...
    // Всі стилі CSS для гарбуза та анімації
    const HALLOWEEN_STYLES = `<style>
        .head__logo-halloween {
            position: relative;
            overflow: hidden;
            border-radius: 50%;
            background: #202020;
        ...
        .head__logo-halloween svg {
            width: 100%;
            height: 100%;
            position: absolute;
            left: 0;
            top: 0;
            filter: drop-shadow(0 0 5px rgba(242, 117, 3, 0.8));
            transition: transform 0.5s ease;
        }

        .head__logo-halloween:hover svg {
            transform: rotate(-5deg) scale(1.05);
        }
        
        .head__logo-halloween-fire {
            position: absolute;
            bottom: 0;
            left: 50%;
            width: 60%;
            height: 50%;
            background: radial-gradient(circle at center 80%, #ff8c00 0%, transparent 70%);
            opacity: 0.8;
    ...
    /**
     * Основна функція для ініціалізації плагіна.
     */
    function initHalloweenPlugin() {
        
        // 1. Створення нового логотипу з SVG-гарбузом
        let logo = $(`<div class="head__logo-icon head__logo-halloween">
        ...
        // 2. Заміна стандартного логотипу
        $('.head .head__logo-icon').replaceWith(logo);

        // 3. Динамічне додавання CSS-стилів
        $('body').append(HALLOWEEN_STYLES);

        // 4. Додавання обробника події (відкриття/закриття меню)
        logo.on('mousedown', () => {
            Lampa.Menu.toggle();
        });
    }

    // --- Логіка очікування готовності Lampa ---

    if (window.appready) {
        // Якщо Lampa вже готова, запускаємо плагін негайно
        initHalloweenPlugin();
    } else {
        // Якщо ні, підписуємося на подію 'ready'
