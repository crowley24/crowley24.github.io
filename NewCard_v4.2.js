(function () {        
        ...
        Object.keys(TRANSLATIONS).forEach(key => {        
            Lampa.SettingsApi.addParam({        
                ...
                param: {        
                    name: key,        
                    type: 'trigger',        
                    default: defaults[key.replace('settings_cas_', 'cas_')]        
                },        
                field: {        
                    name: TRANSLATIONS[key]        
                },        
                onChange: () => {        
                    if (key.includes('scale') || key.includes('gap') || key.includes('size')) {        
                        updateCSSVariables();        
                    }        
                }        
            });        
    ...
    function updateCSSVariables() {        
        ...
        root.style.setProperty('--cas-logo-scale', Lampa.Storage.get('cas_logo_scale') / 100);        
        root.style.setProperty('--cas-blocks-gap', Lampa.Storage.get('cas_blocks_gap') + 'px');        
        root.style.setProperty('--cas-meta-size', Lampa.Storage.get('cas_meta_size') + 'em');        
    }        
    ...
    function addStyles() {        
        ...
        .full-start-new { display: flex; height: 100%; }        
        .full-start-new__body { display: flex; width: 100%; }        
        .full-start-new__left { flex-shrink: 0; width: 30%; }        
        .full-start-new__right { flex-grow: 1; position: relative; }        
        .left-title__content { padding: 3em 4em; height: 100%; display: flex; flex-direction: column; justify-content: flex-end; }        
        .full-start-new__buttons { display: flex; gap: 15px; margin-top: var(--cas-blocks-gap); flex-wrap: wrap; }        
        .full-start__button { display: flex; align-items: center; gap: 8px; padding: 12px 20px; background: rgba(255,255,255,0.1); border-radius: 8px; color: white; cursor: pointer; transition: all 0.3s; }        
        .full-start__button:hover { background: rgba(255,255,255,0.2); }        
        .full-start__button.focus { background: rgba(255,255,255,0.3); }        
        .cas-logo-container { opacity: 0; transform: translateY(20px); animation: fadeInUp 0.6s ease forwards; }        
        .cas-studios-row { opacity: 0; transform: translateY(20px); animation: fadeInUp 0.6s ease 0.1s forwards; }        
        .cas-ratings-line { opacity: 0; transform: translateY(20px); animation: fadeInUp 0.6s ease 0.2s forwards; }        
        .cas-description { opacity: 0; transform: translateY(20px); animation: fadeInUp 0.6s ease 0.3s forwards; }        
        .cas-quality-row { opacity: 0; transform: translateY(20px); animation: fadeInUp 0.6s ease 0.4s forwards; }        
    ...
    function setCachedData(id, data) {        
        ...
        // Обмеження кешу до 100 записів для стабільності  
            ...
            Lampa.Parser.get({ search: data.title || data.name, movie: data, page: 1 }, (res) => {        
                try {        
                    ...
                    if (items && Array.isArray(items) && items.length > 0) {        
                        ...
                        const qualityHtml = [];        
                        if (b.res) qualityHtml.push(`<div class="cas-quality-item"><img src="${QUALITY_ICONS[b.res]}"></div>`);        
                        if (b.hdr) qualityHtml.push(`<div class="cas-quality-item"><img src="${QUALITY_ICONS['HDR']}"></div>`);        
                        if (b.dv) qualityHtml.push(`<div class="cas-quality-item"><img src="${QUALITY_ICONS['Dolby Vision']}"></div>`);        
                        if (b.ukr) qualityHtml.push(`<div class="cas-quality-item"><img src="${QUALITY_ICONS['UKR']}"></div>`);        
                          
                        render.find('.cas-quality-row').html(qualityHtml.join('')).show();        
                    }        
                });        
            }        
        } catch (e) {        
