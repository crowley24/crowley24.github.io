  ...
  function getStudioLogos(movie) {
    ...
    if (movie && movie.production_companies) {
      movie.production_companies.forEach(function(co) {
        if (co.logo_path) {
          // ЗМІНЕНО: Видалено фільтр brightness(0) invert(1), додано легку тінь для видимості
          html += '<div class="quality-badge studio-logo" style="margin-right: 12px; display: inline-block; vertical-align: middle;">' +
                    '<img src="' + TMDB_IMAGE_URL + co.logo_path + '" title="' + co.name + '" style="opacity: 1; height: 1.8em; width: auto; margin-top: -2px; filter: drop-shadow(0 0 2px rgba(0,0,0,0.5));">' +
                  '</div>';
        }
      });
    }
