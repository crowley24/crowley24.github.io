...
(function() {        
  ...
  function modifyCardifyStyles() {          
    ...
    const trailerSize = Lampa.Storage.field('cardify_trailer_size') || '40';          
    ...
    style.textContent = `  
      ...
      .cardify-trailer__youtube {            
        ...
        pointer-events: auto !important;  /* Дозволити взаємодію */  
      ...
      /* Стан фокусу для пульта */  
      .cardify-trailer__youtube.focus {  
        opacity: 1 !important;  
      ...
      .cardify-trailer__youtube iframe {            
        ...
        pointer-events: none !important;  /* iframe не перехоплює події */  
      }            
      ...
      /* Панель керування при наведенні */  
      .cardify-trailer__controls-panel {  
        position: absolute;  
        bottom: 0;  
        left: 0;  
        right: 0;  
      ...
      .cardify-trailer__youtube:hover .cardify-trailer__controls-panel,  
      .cardify-trailer__youtube.focus .cardify-trailer__controls-panel {  
        opacity: 1;  
        pointer-events: auto;  
      }  
        
      .cardify-trailer__controls-panel button {  
      ...
      .cardify-trailer__controls-panel button:hover {  
...
`;          
  ...
  function applyClassToTrailers(trailerSize) {        
    document.querySelectorAll('.cardify-trailer__youtube').forEach(el => {        
      ...
      el.classList.add('selector');  // Додати клас для навігації  
      makeTrailerInteractive(el);  
      console.log('[Cardify] Додано клас size-' + trailerSize + ' до існуючого трейлера');        
    });        
  }  
    
  function makeTrailerInteractive(trailer) {  
    if (trailer.dataset.interactive) return;  
    trailer.dataset.interactive = 'true';  
      
    // Додати панель керування  
    const controlsPanel = document.createElement('div');  
    controlsPanel.className = 'cardify-trailer__controls-panel';  
    controlsPanel.innerHTML = `  
      <button class="trailer-fullscreen">Повний екран</button>  
      <button class="trailer-mute">Звук</button>  
      <button class="trailer-close">Закрити</button>  
    `;  
    trailer.appendChild(controlsPanel);  
      
    // Отримати YouTube player  
    const iframe = trailer.querySelector('iframe');  
      
    // Обробники jQuery подій для пульта  
    $(trailer).on('hover:enter', function() {  
      $(this).addClass('focus');  
    });  
      
    $(trailer).on('hover:focus', function() {  
      $(this).addClass('focus');  
    });  
      
    $(trailer).on('hover:blur', function() {  
      $(this).removeClass('focus');  
    });  
      
    // Клік для повного екрану  
    trailer.addEventListener('click', function(e) {  
      if (e.target.closest('.cardify-trailer__controls-panel')) return;  
    ...
    // Обробники кнопок  
    const fullscreenBtn = controlsPanel.querySelector('.trailer-fullscreen');  
    const muteBtn = controlsPanel.querySelector('.trailer-mute');  
    const closeBtn = controlsPanel.querySelector('.trailer-close');  
    ...
    muteBtn.addEventListener('click', function(e) {  
      ...
      if (iframe && iframe.contentWindow) {  
        // Відправити команду YouTube API  
