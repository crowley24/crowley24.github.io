(function () {
    ...
    function applyStyles() {
        var oldStyle = document.getElementById('tv-interface-clean-v7');
        if (oldStyle) oldStyle.parentNode.removeChild(oldStyle);

        var style = document.createElement('style');
        style.id = 'tv-interface-clean-v7';
        ...
        var css = `
            @media screen and (min-width: 481px) {
                ...
                .full-start-new__right {
                    ...
                    justify-content: center !important; /* Центруємо всю пачку елементів */
                ...
                .full-start-new__title { order: 1; margin-bottom: 10px !important; }
                .plugin-tv-meta { order: 2; margin-bottom: 10px; font-size: 1.3rem; display: flex; gap: 15px; color: rgba(255,255,255,0.8); }
                .plugin-tv-plot { 
                    order: 3; 
                    margin-bottom: 20px; 
                    font-size: 1.1rem; 
                ...
                /* Рейтинги, статус та вік (ті, що накладалися) */
                ...
                /* Прибираємо стандартний слоган, щоб не заважав */
                .full-start-new__tagline { display: none !important; }
            }
        `;
    ...
    function renderPlot(movie, $container) {
        $container.find('.plugin-tv-meta, .plugin-tv-plot').remove();
        ...
        var meta = $('<div class="plugin-tv-meta">' + 
            (year ? '<span>' + year + '</span>' : '') + 
        ...
        // Вставляємо після титулу, щоб дотриматися черговості order
        meta.insertAfter($container.find('.full-start-new__title'));
        plot.insertAfter(meta);
    }
    ...
    function loadLogo(movie, $container) {
        ...
        if (!$title.find('img').length) {
            $title.html('<img src="" style="opacity:0; transition: opacity 0.4s">');
        }
