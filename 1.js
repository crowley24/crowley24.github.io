(function () {              
    ...
    :root { --cas-logo-scale: 1; --cas-blocks-gap: 30px; --cas-meta-size: 1.3em; --cas-anim-curve: cubic-bezier(0.2, 0.8, 0.2, 1); } [1](#31-0)   
    ...
    .cas-logo { text-align: center; margin-bottom: calc(var(--cas-blocks-gap) * 0.8); }  
    .cas-logo img { max-height: 120px; object-fit: contain; }  
    .cas-logo-text { font-size: 2.5em; font-weight: 800; text-transform: uppercase; line-height: 1.2; }  
    .cas-logo-subtitle { font-size: 1.2em; opacity: 0.8; margin-top: 0.5em; }  
    .cas-quality-badges { display: flex; gap: 8px; margin-bottom: var(--cas-blocks-gap); }  
    .cas-quality-badge { background: rgba(255, 255, 255, 0.2); padding: 4px 8px; border-radius: 4px; font-size: 0.8em; }  
    .cas-ratings { display: flex; gap: 15px; margin-bottom: var(--cas-blocks-gap); }  
    .cas-rating { display: flex; align-items: center; gap: 5px; }  
    .cas-rating img { height: 16px; }  
    .cas-meta-info { display: flex; flex-wrap: wrap; gap: 10px; margin-bottom: var(--cas-blocks-gap); }  
    .cas-meta-item { background: rgba(255, 255, 255, 0.1); padding: 6px 12px; border-radius: 20px; font-size: 0.9em; }  
    .cas-description { margin-bottom: var(--cas-blocks-gap); line-height: 1.6; }  
    .cas-actors { display: flex; flex-wrap: wrap; gap: 10px; margin-bottom: var(--cas-blocks-gap); }  
    .cas-actor { display: flex; align-items: center; gap: 8px; background: rgba(255, 255, 255, 0.05); padding: 8px 12px; border-radius: 8px; }  
    .cas-actor img { width: 32px; height: 32px; border-radius: 50%; object-fit: cover; }  
    .cas-actor-name { font-size: 0.9em; }  
    .cas-actor-role { font-size: 0.8em; opacity: 0.7; }  
    .cas-actions { display: flex; gap: 10px; margin-top: var(--cas-blocks-gap); }  
    .cas-action-button { background: rgba(255, 255, 255, 0.2); border: none; padding: 12px 24px; border-radius: 8px; color: white; cursor: pointer; transition: background 0.3s; }  
    .cas-action-button:hover { background: rgba(255, 255, 255, 0.3); }  
    .cas-slideshow { position: absolute; top: 20px; right: 20px; display: flex; gap: 8px; }  
    .cas-slideshow-dot { width: 8px; height: 8px; border-radius: 50%; background: rgba(255, 255, 255, 0.5); cursor: pointer; transition: background 0.3s; }  
    .cas-slideshow-dot.active { background: white; }  
    .cas-reactions { display: flex; gap: 10px; margin-top: var(--cas-blocks-gap); }  
    .cas-reaction { background: rgba(255, 255, 255, 0.1); border: none; padding: 8px 16px; border-radius: 20px; color: white; cursor: pointer; font-size: 1.2em; transition: background 0.3s; }  
    .cas-reaction:hover { background: rgba(255, 255, 255, 0.2); }  
    .cas-reaction.active { background: rgba(255, 255, 255, 0.3); }  
    .cas-settings-button { position: absolute; top: 20px; left: 20px; background: rgba(0, 0, 0, 0.5); border: none; padding: 10px; border-radius: 50%; color: white; cursor: pointer; transition: background 0.3s; }  
    .cas-settings-button:hover { background: rgba(0, 0, 0, 0.7); }  
    .cas-settings-panel { position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); background: rgba(0, 0, 0, 0.9); padding: 20px; border-radius: 10px; color: white; z-index: 1000; }  
    .cas-settings-panel h3 { margin-top: 0; }  
    .cas-setting { margin-bottom: 15px; }  
    .cas-setting label { display: block; margin-bottom: 5px; }  
    .cas-setting input, .cas-setting select { width: 100%; padding: 5px; border-radius: 3px; border: none; }  
    .cas-close-button { position: absolute; top: 10px; right: 10px; background: none; border: none; color: white; font-size: 20px; cursor: pointer; }  
    .cas-zoom-controls { position: absolute; bottom: 20px; right: 20px; display: flex; gap: 10px; }  
    .cas-zoom-button { background: rgba(0, 0, 0, 0.5); border: none; padding: 10px; border-radius: 50%; color: white; cursor: pointer; transition: background 0.3s; }  
    .cas-zoom-button:hover { background: rgba(0, 0, 0, 0.7); }  
    .cas-quality-item img { height: 1.1em; }  
