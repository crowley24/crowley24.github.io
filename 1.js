...
function pluginPage(object) {  
	...
	// Додаємо шаблон для лівої панелі категорій  
	Lampa.Template.add(plugin.component + '_categories_panel', `  
		<div class="${plugin.component}__categories panel--left">  
			<div class="panel__body"></div>  
		</div>  
	`);  
	var categoriesPanel = Lampa.Template.get(plugin.component + '_categories_panel');  
  ...
  this.build = function (catalog) {  
		...
		// Наповнюємо ліву панель категорій  
		var panelBody = categoriesPanel.find('.panel__body');  
		panelBody.empty();  
		lists[object.id].groups.forEach(function(group){  
			var item = $('<div class="menu__item selector"><div class="menu__text">' + group.title + '</div></div>');  
		...
		var info = Lampa.Template.get(plugin.component + '_info_radio');  
	...
	this.append = function (data) {  
		...
		data.forEach(function (channel) {  
			bulkFn(channel);  
			if (!!channel['epgId']) {  
				if (catEpg.indexOf(channel['epgId']) === -1) catEpg.push(channel['epgId']);  
			}  
		});  
	};
	...
	this.start = function () {  
		...
		Lampa.Controller.add('content', {  
			...
			left: function left() {  
				...
				else Lampa.Controller.collectionSet(categoriesPanel);  
			},  
			right: function right() {  
				if (Lampa.Controller.enabled().name === 'categories') {  
					Lampa.Controller.toggle('content');  
