(function () {              
    ...
    :root { --cas-logo-scale: 1; --cas-blocks-gap: 30px; --cas-meta-size: 1.3em; --cas-anim-curve: cubic-bezier(0.2, 0.8, 0.2, 1); } [1](#18-0)   
      
    /* МАКСИМАЛЬНО ЯСКИРАВИЙ ФОН - ПЕРЕКРИВАЄМО ВСЕ */  
    ...
    /* ВИДАЛЯЄМО ВСІ МОЖЛИВІ ГРАДІЄНТИ */  
    *::before,  
    *::after {  
    ...
    /* СПЕЦИФІЧНІ СЕЛЕКТОРИ ДЛЯ LAMPA */  
    ...
    .left-title__content::after,  
    .full-start__left::before,  
    .full-start__left::after {  
    ...
    /* ПЕРЕКРИВАЄМО БУДЬ-ЯКІ ДИНАМІЧНІ СТИЛІ */  
    [style*="gradient"],  
    [style*="rgba"] {  
    ...
    /* ЗАБЕЗПЕЧУЄМО ЯСКРАВІСТЬ ПРИ ПРОКРУТЦІ */  
    ...
    .full-start-new__buttons { display: flex !important; flex-direction: row !important; gap: 20px; margin-top: calc(var(--cas-blocks-gap) * 0.8); }  
    .full-start__button.button--play { order: 1; }  
    .full-start__button.button--trailer { order: 2; }  
    ...
    .left-title .full-start-new__buttons .full-start__button { background: transparent !important; color: rgba(255,255,255,0.6) !important; display: flex; align-items: center; gap: 10px; transition: all 0.3s ease; will-change: transform; border: 2px solid transparent !important; border-radius: 8px !important; padding: 8px 16px !important; }  
    .left-title .full-start-new__buttons .full-start__button.focus { color:#fff!important; transform:scale(1.04); background:rgba(255,255,255,0.12)!important; border-color:rgba(255,255,255,0.25)!important; }  
    ...
    .cas-logo img { background: transparent !important; border: none !important; max-width: 450px; max-height: 200px; width: auto; height: auto; transform: scale(var(--cas-logo-scale)); transform-origin: left center; display: block; object-fit: contain; }  
    .cas-studio-item { height: 2.3em !important; display: flex; align-items: center; justify-content: center; background: rgba(255, 255, 255, 0.1); padding: 4px 8px; border-radius: 6px; }  
    .cas-studio-item img { height: 100%; width: auto; object-fit: contain; filter: drop-shadow(0 2px 4px rgba(0,0,0,0.2)); opacity: 1; transition: all 0.3s ease; }  
    ...
    @keyframes casKenBurnsParallax { 0% { transform: scale(1.1) translateY(0px) translateX(0px); } 25% { transform: scale(1.13) translateY(-10px) translateX(-5px); } 50% { transform: scale(1.15) translateY(-20px) translateX(5px); } 75% { transform: scale(1.13) translateY(-10px) translateX(-3px); } 100% { transform: scale(1.1) translateY(0px) translateX(0px); } }  
    body.cas--zoom-enabled .full-start__background.loaded { animation: casKenBurnsParallax 50s ease-in-out infinite !important; will-change: transform; }  
    .full-start__background img { transform: translateZ(0); -webkit-transform: translateZ(0); }  
    </style>`;                      
        ...
        if (event.type === 'complite') {                      
            ...
            if (data && data.id) {                      
                ...
                if (cached) processImagesWrapper(cached);                      
                    ...
                    const ajaxRequest = $.getJSON(imagesUrl, (res) => {                      
            ...
            // Агресивне очищення градієнтів  
            setTimeout(() => {  
                // Видаляємо всі псевдоелементи через CSS  
                ...
                // Примусово очищуємо всі елементи  
                ...
                // Видаляємо всі темні оверлеї  
                render.find('[style*="gradient"], [style*="rgba"]').each(function() {  
                    this.style.setProperty('background', 'transparent', 'important');  
                    this.style.setProperty('background-image', 'none', 'important');  
                });  
                  
                console.log('Aggressive gradient removal completed');  
            }, 500);  
        }                      
    });                      
}
