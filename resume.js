...
(function () {  
    ...
    function playEntrySame(entry) {  
        ...
        if (!entry || !entry.url) {  
            console.warn('[resume] Недійсний запис або відсутня URL');  
            Lampa.Noty.show('Resume: неможливо продовжити, дані пошкоджені', { time: 3000 });  
            return;  
        }  
        ...
        // Забезпечення дійсних значень для всіх полів  
        var playData = {  
            url: entry.url,  
    ...
    function playEntryNext(entry, nextItem) {  
        if (!nextItem || nextItem.lazy || !nextItem.url) {  
            Lampa.Noty.show('Resume: наступна серія потребує звичайного запуску — відкрийте джерело', { time: 4000 });  
            playEntrySame(entry);  
            return;  
        }  
